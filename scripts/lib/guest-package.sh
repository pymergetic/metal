...
#   /mods/apps/<name>.wasm           (opt-in apps, e.g. doom — not auto-packaged here)
#
# Kernel MicroPython lives in core (metal/py); scripts/samples under mods/py/.
# Guest CPython (python.wasm + stdlib) packaging was removed.
#
# Knobs (env, default 1):
...
pm_guest_package_compose() {
	...
	if pm_guest_package_is_fresh; then
		echo "guest-package: up to date (${PM_GUEST_PACKAGE}; tests=${PM_METAL_GUEST_TESTS})"
		return 0
	fi
	...
	echo "guest-package -> ${PM_GUEST_PACKAGE} (tests=${PM_METAL_GUEST_TESTS})"
}
