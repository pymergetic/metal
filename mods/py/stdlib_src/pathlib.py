...
# Metal patch: upstream calls bare `open()` throughout this file, assuming
# a VFS-mounted builtin -- this build has none (py_port_stubs.c's
# mp_builtin_open_obj is a permanent OSError stub, see io.py's own note),
# so every open() call below is Metal's own io.open() instead.
from io import open

# Metal patch: no `micropython` builtin module in this build (MICROPY_PY_
# MICROPYTHON defaults off at MICROPY_CONFIG_ROM_LEVEL_MINIMUM, not flipped
# -- nothing else here needs it), so `from micropython import const` would
# be an ImportError; a plain literal below does the same job (see zlib.py's
# own note on why bare const(x) itself doesn't need MICROPY_COMP_CONST
# fixed either way).
_SEP = "/"
