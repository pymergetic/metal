 ...
 */
static int MetalDoomScaleMouse(int v)
{
  int a;
  int s;
  int gw;
...
static void MetalDoomApplyBindings(void)
{
...
static void MetalDoomEnsurePointerLock(void)
{
  uint32_t surf;
...
static void PostKey(int pressed, unsigned char key)
{
  event_t event;
  ...
  if (pressed) {
    ...
    event.data1 = key;
    event.data2 =
      (shiftdown > 0 && key >= 'a' && key <= 'z') ? (int)toupper((unsigned char)key) : (int)key;
    if (event.data1 != 0) {
      D_PostEvent(&event);
    }
  } else {
...
static void DrainMouse(void)
{
  ...
  while (pm_metal_input_poll_pointer((uint32_t)(uintptr_t)&ev) != 0) {
    got             = 1;
    ...
    if (pm_metal_input_pointer_locked() == 0 && (buttons & 1) != 0 && (prev & 1) == 0) {
