 ...
 *
 * Queue S16LE stereo 22050 into pm_metal_audio_*. Feed from wall-clock, not
 * one slice per Tick: Tick dips (present hitch / wipe freeze) would underrun
 * → QEMU "Resetting rate control" → host hitch → worse present/FPS.
 * No stem drain await (that serialized frames behind broken QEMU audio).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define MIX_HZ         22050
#define MIX_FRAMES     512 /* mix/queue quantum */
#define MIX_BYTES      (MIX_FRAMES * 2 * (int)sizeof(int16_t))
#define MIX_LEAD_MS    80u
#define MIX_CATCHUP_MS 40u
...
static uint64_t                s_audio_start_us;
static uint64_t                s_queued_frames;
...
static metal_sfx_t *CacheSFX(sfxinfo_t *sfxinfo)
{
  byte        *data;
...
static boolean Metal_Init(boolean use_sfx_prefix)
{
  int i;
  ...
  s_stream = pm_metal_audio_open(PM_METAL_AUDIO_FMT_S16LE_STEREO_22050, (uint32_t)(MIX_FRAMES * 8));
...
static int Metal_GetSfxLumpNum(sfxinfo_t *sfxinfo)
{
  char namebuf[9];
...
static void Metal_UpdateSoundParams(int channel, int vol, int sep)
{
  int left;
  int right;
...
static int Metal_StartSound(sfxinfo_t *sfxinfo, int channel, int vol, int sep)
{
  metal_sfx_t *sfx;
...
static void Metal_StopSound(int channel)
{
  if (channel < 0 || channel >= NUM_CHANNELS) {
    return;
  }
...
static boolean Metal_SoundIsPlaying(int channel)
{
  if (channel < 0 || channel >= NUM_CHANNELS) {
    return false;
  }
...
static void MetalMixSlice(int nframes)
{
  int i;
  int f;

  if (nframes <= 0 || nframes > MIX_FRAMES) {
    return;
  }

  memset(s_mix, 0, (size_t)nframes * 2u * sizeof(int16_t));
  ...
  for (i = 0; i < NUM_CHANNELS; i++) {
    ...
    for (f = 0; f < nframes; f++) {
      ...
      idx = ch->pos * 2;
      l   = (int32_t)s_mix[f * 2] + (((int32_t)sfx->pcm[idx] * ch->vol_l) / 255);
      r   = (int32_t)s_mix[f * 2 + 1] + (((int32_t)sfx->pcm[idx + 1] * ch->vol_r) / 255);
      if (l > 32767) {
        l = 32767;
      }
      if (l < -32768) {
...
static void Metal_Update(void)
{
  ...
  if (s_audio_start_us == 0) {
    s_audio_start_us = now;
    s_queued_frames  = 0;
  }

  lead   = ((uint64_t)MIX_HZ * (uint64_t)MIX_LEAD_MS) / 1000ull;
  target = ((now - s_audio_start_us) * (uint64_t)MIX_HZ) / 1000000ull;
  target += lead;
  if (target <= s_queued_frames) {
    return;
  }

  owe       = target - s_queued_frames;
  max_catch = ((uint64_t)MIX_HZ * (uint64_t)MIX_CATCHUP_MS) / 1000ull;
  if (owe > max_catch) {
    s_queued_frames = target - max_catch;
    owe             = max_catch;
  }

  slices = 0;
  while (owe > 0 && slices < 8u) {
    int      nframes;
    uint32_t nbytes;
    uint32_t placed;
    uint32_t got;

    nframes = (owe > (uint64_t)MIX_FRAMES) ? MIX_FRAMES : (int)owe;
    MetalMixSlice(nframes);
    nbytes = (uint32_t)nframes * 2u * (uint32_t)sizeof(int16_t);
    placed = pm_metal_audio_queue(s_stream, (uint32_t)(uintptr_t)s_mix, nbytes);
    got    = placed / (2u * (uint32_t)sizeof(int16_t));
    if (got == 0u) {
      break;
    }

    s_queued_frames += got;
    owe = (got < (uint32_t)nframes) ? 0 : (owe - got);
    slices++;
  }
}

static void Metal_CacheSounds(sfxinfo_t *sounds, int num_sounds)
{
  (void)sounds;
  (void)num_sounds;
}
...
sound_module_t DG_sound_module = {
  s_sfx_devices,           arrlen(s_sfx_devices), Metal_Init,
  Metal_Shutdown,          Metal_GetSfxLumpNum,   Metal_Update,
  Metal_UpdateSoundParams, Metal_StartSound,      Metal_StopSound,
  Metal_SoundIsPlaying,    Metal_CacheSounds,
};
...
static void Music_SetVolume(int volume)
{
  (void)volume;
}
...
static void *Music_RegisterSong(void *data, int len)
{
  (void)data;
  (void)len;
  return (void *)1;
...
static boolean Music_IsPlaying(void)
{
  return false;
}
...
music_module_t DG_music_module = {
  s_mus_devices,  arrlen(s_mus_devices), Music_Init,         Music_Shutdown,       Music_SetVolume,
  Music_Pause,    Music_Resume,          Music_RegisterSong, Music_UnRegisterSong, Music_PlaySong,
  Music_StopSong, Music_IsPlaying,       Music_Poll,
};
