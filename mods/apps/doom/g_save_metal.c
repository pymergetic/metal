...
char *__wrap_M_GetSaveGameDir(char *iwadname)
{
  (void)iwadname;
...
void __wrap_M_ReadSaveStrings(void)
{
  int i;
  ...
  for (i = 0; i < LOAD_END; i++) {
    if (s_slot_used[i]) {
      M_StringCopy(savegamestrings[i], s_slot_str[i], SAVESTRINGSIZE);
      LoadMenu[i].status = 1;
    } else {
...
void __wrap_G_SaveGame(int slot, char *description)
{
  s_save_slot = slot;
  if (description != NULL) {
