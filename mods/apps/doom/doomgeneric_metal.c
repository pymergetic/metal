...
static pm_metal_gfx_surface_h MetalDoomPickSurface(void)
{
  ...
  if (pid != PM_METAL_PROCESS_ID_INVALID) {
    ...
    if (kind == (uint32_t)PM_METAL_PROC_UI_TAB && surf != PM_METAL_GFX_SURFACE_INVALID &&
  ...
  if (tab != PM_METAL_UI_HANDLE_INVALID && tab != pm_metal_ui_console_handle()) {
...
void DG_Init(void)
{
  ...
  s_windowed = (surf != PM_METAL_GFX_SURFACE_DEFAULT && surf != PM_METAL_GFX_SURFACE_INVALID);
...
void DG_DrawFrame(void)
{
  ...
  surf = s_windowed ? MetalDoomPickSurface() : PM_METAL_GFX_SURFACE_DEFAULT;
  pm_metal_gfx_set_surface(surf);
...
static unsigned char MetalKeyToDoom(pm_metal_keycode_t code)
{
  switch (code) {
