...
static int bytes_eq(const uint8_t *buf, uint32_t n, const char *exp, uint32_t elen)
{
  uint32_t i;
...
pm_metal_status_t pm_metal_guest_step(pm_metal_async_handle_t self_h)
{
  ...
  case 1:
    s->n = pm_metal_fs_result(self_h);
    if (s->n != 1u || s->st.size != FS_EXPECT_LEN || s->st.type != PM_METAL_FS_TYPE_FILE) {
      return PM_METAL_ERROR;
    }
  ...
  case 2:
    s->fh = (pm_metal_fs_h)pm_metal_fs_result(self_h);
  ...
  case 5:
    ...
    s->aw = pm_metal_fs_open_async(FS_DIR, PM_METAL_FS_O_RDONLY | PM_METAL_FS_O_DIRECTORY);
    if (s->aw == PM_METAL_ASYNC_HANDLE_INVALID) {
      return PM_METAL_ERROR;
    }
    s->step = 6;
...
int32_t pm_metal_mod_on_load(void)
{
  ...
  if (pm_metal_mod_register_cmd("async_fs_fd", "run", "async_fs_fd mod command") != 0) {
    return -1;
  }
