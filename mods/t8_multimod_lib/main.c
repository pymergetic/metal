 ...
 * to import directly (see mods/t9_multimod_app) — proving WAMR's own
 ...
 * file — see scripts/build-mod.sh), i.e. no _start/main of its own: WAMR's
 ...
 * comment) the moment mods/t9_multimod_app's own import section first
 * names it; nothing here has to know that's how it's found. Not run
 * standalone by scripts/verify-linux.sh's own driver list for exactly
 * that reason — a reactor has no entry point that run()'s own
