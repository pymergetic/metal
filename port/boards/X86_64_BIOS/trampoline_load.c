 ...
 * overlaying Metal at 64MiB (load window starts at image paddr).
 */
#include <stdint.h>
...
uint32_t trampoline_load_elf64(void)
{
  ...
  {
    /* Must match link.ld image base (.bootinfo at start of Metal ELF64). */
    uint32_t *bi = (uint32_t *)phys_ptr(0x4000000u);
