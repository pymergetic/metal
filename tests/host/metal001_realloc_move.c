...
static size_t metal_copy_len(size_t old_size, size_t new_size)
{
  return (old_size < new_size) ? old_size : new_size;
}

/* Same copy rule as mem.c after tlsf_realloc fails and a new block is alloc'd. */
static void *metal_realloc_move_copy(void *dst, void *ptr, size_t old_size, size_t new_size)
{
...
static int test_guard_page(void)
{
  ...
  map = mmap(NULL, (size_t)page * 2, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (map == MAP_FAILED) {
...
static int test_tlsf_move_no_blocker_bleed(void)
{
  enum {
    POOL = 64 * 1024
  };
