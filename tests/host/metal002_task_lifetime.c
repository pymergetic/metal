...
static int task_spawn_msg(task_t **inbox, size_t *n, size_t cap, task_t *t)
{
  if (t->doomed) {
    return -1;
  }
...
static void task_dispatch_one(task_t **inbox, size_t *n)
{
  task_t *t;
  assert(*n > 0);
  t = inbox[0];
...
static void task_destroy(task_t *t, task_t **inbox, size_t *n)
{
  t->cancelled = 1;
...
static int test_inbox_uaf(void)
{
  task_t *inbox[64];
  size_t  n = 0;
  task_t *t;
...
static int test_timer_retain(void)
{
  task_t *t = task_new();
  task_t *held;
...
static int test_exclusive_await(void)
{
