 ...
 * which is why every mod in scripts/verify linux none keeps working
 ...
 * init()/shutdown() pair. This is what scripts/verify linux none and
 * scripts/verify linux none threads exercise. With `--`, only one .wasm is allowed
 * and remaining args become that guest's argv; `--env=KEY=VAL` (repeatable)
