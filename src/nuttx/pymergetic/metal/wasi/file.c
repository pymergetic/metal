/*
 * NuttX WASI file wraps — live mount-table resolve + virtual proc.
 * Same shape as the linux bind: untagged handles delegate to stock posix
 * via __real_os_* (posix_file_real.c). See docs/MOUNT.md Phase 6b.
 * WAMR's nuttx platform already ships posix_file.c; we still wrap it so
 * Metal virtual /proc + live remount match linux behavioural parity.
 */
#include <fcntl.h>
#include <limits.h>
#include <stdio.h>
...
static __wasi_errno_t pm_metal_proc_open_memfd(pm_metal_mount_proc_hook_fn fn, os_file_handle *out)
{
	...
	ssize_t n;
	size_t off;
	/* NuttX has no memfd_create(); anonymous content via mkstemp + unlink. */
	char path[] = "/tmp/pm_metal_proc_XXXXXX";
...
/*
 * NuttX libc openat()/fstatat()/… rebuild relative paths with
 * F_GETPATH + strlcat and omit the '/' between directory and name
 * (`/v` + `README` → `/vREADME`). Absolute paths skip that join, so
 * live-resolve host ops prefer AT_FDCWD + a correctly joined host path.
 */
static int pm_metal_join_host(const char *base, const char *rel, char *out, size_t out_cap)
{
	const char *node = rel ? rel : "";
	size_t blen;
	int n;

	if (!base || !out || out_cap == 0) {
		return -1;
	}
	while (node[0] == '/') {
		node++;
	...
	if (node[0] == '\0' || strcmp(node, ".") == 0) {
		...
		if (blen > 0 && base[blen - 1] == '/') {
			n = snprintf(out, out_cap, "%s%s", base, node);
		} else {
			n = snprintf(out, out_cap, "%s/%s", base, node);
		}
	}
...
static __wasi_errno_t pm_metal_live_open_host(const char *host_base, const char *remainder, __wasi_oflags_t oflags,
					       ...
					       wasi_libc_file_access_mode read_write_mode, os_file_handle *out)
{
	char abs[PM_METAL_MOUNT_HOST_PATH_MAX];
	const char *rel = (remainder && remainder[0]) ? remainder : ".";

	if (strcmp(rel, ".") == 0) {
		return __real_os_open_preopendir(host_base, out);
	}
	if (pm_metal_join_host(host_base, rel, abs, sizeof(abs)) != 0) {
		return __WASI_ENAMETOOLONG;
	}
	return __real_os_openat(AT_FDCWD, abs, oflags, fs_flags, lookup_flags, read_write_mode, out);
}
			   ...
			   __wasi_lookupflags_t lookup_flags)
{
	...
	}
	{
		...
		if (pm_metal_join_host(r.host_base, rel, abs, sizeof(abs)) != 0) {
			return __WASI_ENAMETOOLONG;
		}
		return __real_os_fstatat(AT_FDCWD, abs, buf, lookup_flags);
	}
}
			      ...
			      size_t *bufused)
{
	...
	return __real_os_readlinkat(AT_FDCWD, abs, buf, bufsize, bufused);
}
...
__wasi_errno_t os_mkdirat(os_file_handle handle, const char *path)
{
	...
	return __real_os_mkdirat(AT_FDCWD, abs);
}
...
__wasi_errno_t os_unlinkat(os_file_handle handle, const char *path, bool is_dir)
{
	...
	return __real_os_unlinkat(AT_FDCWD, abs, is_dir);
}
			    ...
			    const char *new_path)
{
	...
	char old_abs[PM_METAL_MOUNT_HOST_PATH_MAX];
	char new_abs[PM_METAL_MOUNT_HOST_PATH_MAX];
	__wasi_errno_t err;
	...
	if (pm_metal_join_host(orr.host_base, old_rel, old_abs, sizeof(old_abs)) != 0
	    || pm_metal_join_host(nrr.host_base, new_rel, new_abs, sizeof(new_abs)) != 0) {
		return __WASI_ENAMETOOLONG;
	}
	return __real_os_renameat(AT_FDCWD, old_abs, AT_FDCWD, new_abs);
}
			  ...
			  const char *to_path, __wasi_lookupflags_t lookup_flags)
{
	...
	char from_abs[PM_METAL_MOUNT_HOST_PATH_MAX];
	char to_abs[PM_METAL_MOUNT_HOST_PATH_MAX];
	__wasi_errno_t err;
	...
	if (pm_metal_join_host(fr.host_base, from_rel, from_abs, sizeof(from_abs)) != 0
	    || pm_metal_join_host(tr.host_base, to_rel, to_abs, sizeof(to_abs)) != 0) {
		return __WASI_ENAMETOOLONG;
	}
	return __real_os_linkat(AT_FDCWD, from_abs, AT_FDCWD, to_abs, lookup_flags);
}
...
__wasi_errno_t os_symlinkat(const char *old_path, os_file_handle handle, const char *new_path)
{
	...
	return __real_os_symlinkat(old_path, AT_FDCWD, abs);
}
			     ...
			     __wasi_lookupflags_t lookup_flags)
{
	...
	return __real_os_utimensat(AT_FDCWD, abs, access_time, modification_time, fstflags, lookup_flags);
}
