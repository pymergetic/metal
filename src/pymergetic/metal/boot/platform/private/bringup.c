...
int32_t pm_metal_boot_rootfs_proof_src_browse(void);
int32_t pm_metal_reg_proof_reflect(void);
int32_t pm_metal_forge_proof_render(void);
int32_t pm_metal_net_http_microdot_proof_browse(void);
...
int32_t pm_metal_boot_bringup(void)
{
  ...
  /*
   * Registry bootstrap: kernel + floor/product RegMods, then connect.
   * Must run before any always-proxy face call from Rust.
   */
  ...
  /* Pre-EBS gop_blt proof when EFI stashed a live GOP; no-op on BIOS. */
  if (pm_metal_dev_gfx_efi_pre_ebs() != 0) {
    return fail("exp2: gfx pre-ebs failed\n");
  }
  ...
  /* Quiet product init — proofs / guest smokes are stress-only (PLATFORM.md). */
  if (pm_metal_wasm_init() != 0) {
    return fail("exp2: wasm init failed\n");
  }

#if defined(EXP2_STRESS) && EXP2_STRESS
  /* Proof battery before tree so emitters can show await / wasm / http health. */
  if (pm_metal_boot_rootfs_proof_src_browse() != 0) {
    return fail("exp2: src browse proof failed\n");
  }
  if (pm_metal_reg_proof_reflect() != 0) {
    return fail("exp2: reg reflect proof failed\n");
  }
  pm_metal_log((const uint8_t *)"reg reflect ok");
  if (pm_metal_forge_proof_render() != 0) {
    return fail("exp2: forge render proof failed\n");
  }
  pm_metal_log((const uint8_t *)"forge render ok");
  if (pm_metal_trust_proof() != 0) {
    return fail("exp2: trust proof failed\n");
  }
  {
    int32_t ok;
    ...
    ok = (pm_metal_py_proof_concurrency() == 0) ? 1 : 0;
    pm_metal_boot_tree_note_concurrency(ok);
    if (ok == 0) {
      return fail("exp2: py proof_concurrency failed\n");
    }
    ...
    if (pm_metal_wasm_proof_stress() != 0) {
      return fail("exp2: wasm proof_stress failed\n");
    }
    if (pm_metal_reg_call0((const uint8_t *)"tests.wasm_hello",
                           (const uint8_t *)"ready") != 0) {
      return fail("exp2: host->wasm reg_call0 failed\n");
    }
    if (pm_metal_net_http_microdot_proof_browse() != 0) {
      return fail("exp2: doc browse proof failed\n");
    }
    if (pm_metal_wasm_proof_fetch() != 0) {
      return fail("exp2: wasm fetch proof failed\n");
    }
  }
#endif
...
#ifdef CONFIG_PM_METAL_PY_ENABLE_REPL_SHELL
  /* Interactive REPL after tree (forge's ready needle). Use
   * `forge run --interactive` to keep QEMU alive past ready. */
  if (pm_metal_py_shell_start() != 0) {
    return fail("exp2: py shell start failed\n");
  }
#endif
  ...
  /* Stay alive: runners keep ssh/http/REPL pumping. */
