...
#define PM_METAL_RTC_DEFAULT 4u
...
/* The table of bound clocks, widened a step at a time under the device
 * knob instead of reserved at its ceiling: a seat that binds one clock
...
static struct pm_metal_rtcdev *s_dev;
 ...
 * when this seat is already carrying every clock it may. */
static int32_t dev_slot(void) {
    ...
    if (!PM_UTIL_LIMIT_ROOM(pm_metal_drivers_rtc_limit_device, s_dev_used)) {
        return -1;
    }
    ...
    grown = pm_util_limits_grow(s_arena, s_dev, &cap, (uint32_t)sizeof(*s_dev), &pm_metal_drivers_rtc_limit_device);
...
int64_t pm_metal_drivers_rtc_get(int32_t h) {
    if (h < 0 || (uint32_t)h >= s_dev_cap || !s_dev[h].used || s_dev[h].ops.get == NULL) {
        return -1;
    }
...
int32_t pm_metal_drivers_rtc_set(int32_t h, int64_t unix_s) {
    if (h < 0 || (uint32_t)h >= s_dev_cap || !s_dev[h].used || s_dev[h].ops.set == NULL) {
        return -1;
    }
