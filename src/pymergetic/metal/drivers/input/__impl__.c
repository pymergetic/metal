...
#define PM_METAL_INPUT_DEFAULT 8u
...
/* The table of bound input devices, widened a step at a time under the device
 * knob instead of reserved at its ceiling: a seat that binds one input device
...
static struct pm_metal_indev *s_dev;
static uint32_t s_dev_cap;
static uint32_t s_dev_used;

PM_UTIL_LIMIT_C(pm_metal_drivers_input_limit_device, pymergetic.metal.drivers.input, device,
    PM_METAL_INPUT_DEFAULT, 0u, &s_dev_used);

/* A free row, widening the table if that is what the knob allows. Negative
 * when this seat is already carrying every input device it may. */
static int32_t dev_slot(void) {
    ...
    if (!PM_UTIL_LIMIT_ROOM(pm_metal_drivers_input_limit_device, s_dev_used)) {
        return -1;
    }
    ...
    grown = pm_util_limits_grow(s_arena, s_dev, &cap, (uint32_t)sizeof(*s_dev), &pm_metal_drivers_input_limit_device);
