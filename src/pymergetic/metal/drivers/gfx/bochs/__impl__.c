...
PM_UTIL_LIMIT_C(pm_metal_gfx_bochs_limit_device, pymergetic.metal.drivers.gfx.bochs, device,
    GFX_DEVICE_DEFAULT, 0u, &s_dev_used);
PM_UTIL_LIMIT_C(pm_metal_gfx_bochs_limit_shadow, pymergetic.metal.drivers.gfx.bochs, shadow,
...
static int32_t shadow_fit(struct gfx_fill *d) {
    uint32_t want = pm_metal_gfx_bochs_limit_shadow.soft;
    uint8_t *shadow;
    if (want == 0u) {
        want = pm_metal_gfx_bochs_limit_shadow.dflt;
    }
...
static struct gfx_fill *gfx_row(void) {
    ...
    uint32_t rows = 0;
    if (!PM_UTIL_LIMIT_ROOM(pm_metal_gfx_bochs_limit_device, s_dev_used)) {
        return NULL;
    }
    while (d != NULL) {
        if (!d->used) {
...
int32_t pm_metal_drivers_gfx_bochs_probe(void) {
    ...
    return bochs_attach(PM_METAL_DT_BUS_PLATFORM, 0, 0, 0, unit);
}
