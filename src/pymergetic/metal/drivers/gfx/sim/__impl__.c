/* pymergetic.metal.drivers.gfx.sim — instanced scanout. Present copies RGB888 into a shadow. */
#include "pymergetic/metal/drivers/gfx/sim/__exports__.h"
...
PM_UTIL_LIMIT_C(pm_metal_gfx_sim_limit_device, pymergetic.metal.drivers.gfx.sim, device,
    GFX_DEVICE_DEFAULT, 0u, &s_dev_used);
PM_UTIL_LIMIT_C(pm_metal_gfx_sim_limit_shadow, pymergetic.metal.drivers.gfx.sim, shadow,
...
static int32_t shadow_fit(struct gfx_fill *d) {
    uint32_t want = pm_metal_gfx_sim_limit_shadow.soft;
    uint8_t *shadow;
    if (want == 0u) {
        want = pm_metal_gfx_sim_limit_shadow.dflt;
    }
...
static struct gfx_fill *gfx_row(void) {
    ...
    uint32_t rows = 0;
    if (!PM_UTIL_LIMIT_ROOM(pm_metal_gfx_sim_limit_device, s_dev_used)) {
        return NULL;
    }
    while (d != NULL) {
        if (!d->used) {
...
static int32_t sim_attach(int32_t bus, uint32_t loc0, uint32_t loc1, uint32_t loc2, uint32_t loc3) {
    struct gfx_fill *d;
    int32_t dt;
    if (s_arena == NULL) {
        return -1;
    }
    dt = pm_metal_dt_add(PM_METAL_DT_CLASS_GFX, "sim", bus, loc0, loc1, loc2, loc3);
    if (dt < 0) {
        return -1;
    }
...
int32_t pm_metal_drivers_gfx_sim_init(pm_util_mem_arena_t *arena) {
    if (arena == NULL) {
        return -1;
    }
    s_arena = arena;
...
void pm_metal_drivers_gfx_sim_deinit(void) {
    s_head = NULL;
    s_dev_used = 0;
    s_arena = NULL;
}

int32_t pm_metal_drivers_gfx_sim_probe(void) {
    struct gfx_fill *d;
    uint32_t unit = 0;
    for (d = s_head; d != NULL; d = d->next) {
        if (d->used) {
    ...
    return sim_attach(PM_METAL_DT_BUS_PLATFORM, 0, 0, 0, unit);
}

int32_t pm_metal_drivers_gfx_sim_up(void) {
    struct gfx_fill *d;
    if (s_arena == NULL) {
        return -1;
    }
    ...
    return pm_metal_drivers_gfx_sim_probe() >= 0 ? 0 : -1;
}

#include "pymergetic/wasmmod/guest.h"

PM_MOD_EXPORT_C(pymergetic.metal.drivers.gfx.sim, pm_metal_drivers_gfx_sim_init, pm_metal_drivers_gfx_sim_init, int32_t(pm_util_mem_arena_t *));
PM_MOD_EXPORT_C(pymergetic.metal.drivers.gfx.sim, pm_metal_drivers_gfx_sim_deinit, pm_metal_drivers_gfx_sim_deinit, void(void));
PM_MOD_EXPORT_C(pymergetic.metal.drivers.gfx.sim, pm_metal_drivers_gfx_sim_probe, pm_metal_drivers_gfx_sim_probe, int32_t(void));
PM_MOD_EXPORT_C(pymergetic.metal.drivers.gfx.sim, pm_metal_drivers_gfx_sim_up, pm_metal_drivers_gfx_sim_up, int32_t(void));

PM_MOD_BOOT_C(pymergetic.metal.drivers.gfx.sim, pm_metal_drivers_gfx_sim_init, pm_metal_drivers_gfx_sim_deinit);
PM_MOD_BOOTDEP_C(pymergetic.metal.drivers.gfx.sim, pymergetic.metal.drivers.gfx);
