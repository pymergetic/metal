...
static int32_t case_session_knob(void) {
    ...
    if (pm_metal_net_zenoh_sel(3) != 0) {
        return fail_zenoh("the slot the knob just added is not there");
    }
    /* With a session live the table cannot move: zenoh-pico holds pointers
     * into the slot it was handed. The knob stays where it was. */
    (void)pm_metal_net_zenoh_sel(0);
    if (pm_metal_net_zenoh_peer(ZENOH_ADDR_BE, ZENOH_PORT, 1) != 0) {
    ...
    if (pm_metal_net_zenoh_sel(3) == 0) {
        return fail_zenoh("a slot survived the knob coming back down");
    }
    (void)pm_metal_net_zenoh_sel(0);
    return 0;
}
...
PM_MOD_TEST_C(pymergetic.metal.net.zenoh, "session_knob", case_session_knob);
