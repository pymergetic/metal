...
void metal_db_fd_release(int fd)
{
  ...
  if (g_fds[fd].kind == DB_FD_FILE) {
    if (g_fds[fd].path != NULL) {
      pm_metal_mem_free(g_fds[fd].path);
    }
...
int metal_db_open_path(const char *path, int flags)
{
  ...
  pcopy = (char *)pm_metal_mem_alloc(plen + 1u, PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
  buf   = (uint8_t *)pm_metal_mem_alloc(DB_FILE_MAX, PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
  if (pcopy == NULL || buf == NULL) {
    if (pcopy != NULL) {
      pm_metal_mem_free(pcopy);
    }
    if (buf != NULL) {
...
int metal_db_close(int fd)
{
  ...
  g_push_valid[fd] = 0;
  if (g_fds[fd].kind == DB_FD_FILE && g_fds[fd].writable && g_fds[fd].dirty &&
      g_fds[fd].path != NULL && g_fds[fd].data != NULL) {
    char     path[DB_PATH_MAX];
    uint8_t *data;
    uint32_t len;
    uint32_t n;

    /* Copy out then release the 2KiB FILE buffer before fs_write — hostkey
     * flush was returning EIO and Dropbear m_close treated that as fatal. */
    len = g_fds[fd].data_len;
    if (len > DB_FILE_MAX) {
      len = DB_FILE_MAX;
    }
    strncpy(path, g_fds[fd].path, sizeof(path) - 1u);
    path[sizeof(path) - 1u] = '\0';
    data = (uint8_t *)pm_metal_mem_alloc(len ? len : 1u, PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
    if (data != NULL && len > 0u) {
      memcpy(data, g_fds[fd].data, len);
    }
    metal_db_fd_release(fd);
    n = 0;
    if (data != NULL) {
      n = pm_metal_fs_write(path, data, len);
      pm_metal_mem_free(data);
    }
    if (n != len) {
      pm_metal_logf("sshd: fs_write %s failed (%u/%u)", path, (unsigned)n, (unsigned)len);
    }
    return 0;
  }
