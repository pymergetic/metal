...
class Compiler:
    ...
    def __init__(self, file_in, file_out, indent=0, seq=0, loader=None):
        """Compile one template into Python source.

        :param loader: the owning :class:`Loader`, required for
        ``{% include %}`` resolution. Both call sites pass it (top-level from
        ``Loader.load``, nested includes from a parent compiler). The ``None``
        fallback is only a legacy default; at the ``include`` use below
        ``input_open`` is dereferenced unconditionally, so we narrow it there.
        """
    ...
    def parse_statement(self, stmt):
        ...
        if tokens[0] == "args":
            ...
            assert self.loader is not None  # every Compiler is built with a Loader
