...
static int wait_register(uint32_t h, uint8_t kind, uint32_t min_bytes)
{
    uint32_t i;
...
void pm_metal_net_pump_wake_tcp(void)
{
    ...
    for (i = 0; i < PM_METAL_NET_TCP_WAITS; i++) {
        ...
        if (g_waits[i].kind == WAIT_TCP_EST) {
            ready = pm_metal_net_ip_tcp_established() ? 1 : 0;
        } else if (g_waits[i].kind == WAIT_TCP_RX) {
            if (pm_metal_net_ip_tcp_rx_avail() >= g_waits[i].min_bytes) {
                ready = 1;
            } else if (pm_metal_net_ip_tcp_peer_closed()
...
uint32_t pm_metal_net_await_tcp_established_h(void)
{
    ...
    if (wait_register(h, WAIT_TCP_EST, 0) != 0) {
        pm_metal_async_wake(h);
        return 0;
    }
...
uint32_t pm_metal_net_await_tcp_rx_h(uint32_t min_bytes)
{
    ...
    if (wait_register(h, WAIT_TCP_RX, min_bytes) != 0) {
        pm_metal_async_wake(h);
        return 0;
    }
    pm_metal_net_pump_wake_tcp();
...
{
    uint32_t h = pm_metal_net_await_tcp_established_h();
    uint32_t i;
...
{
    uint32_t h = pm_metal_net_await_tcp_rx_h(min_bytes);
    uint32_t i;
