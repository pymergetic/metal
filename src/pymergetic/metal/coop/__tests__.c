/* pymergetic.metal.coop — host prove (not product exports). */
...
static int32_t fail(const char *why) {
    fprintf(stderr, "metal.coop test: %s\n", why);
    return 1;
}

static pm_metal_coop_status_t step_count_yield(pm_metal_coop_coro_t *self) {
...
static int32_t case_yield_two_tasks(void) {
    uint32_t a = 0;
    uint32_t b = 0;
    count_frame_t *fa = (count_frame_t *)pm_metal_coop_coro_create(step_count_yield, sizeof(*fa));
    count_frame_t *fb = (count_frame_t *)pm_metal_coop_coro_create(step_count_yield, sizeof(*fb));
    if (fa == NULL || fb == NULL) {
        return fail("coro");
    }
    fa->counter = &a;
    ...
    fb->counter = &b;
    fb->target = 4;
    pm_metal_coop_task_t *ta = pm_metal_coop_create_task(&fa->coro);
    pm_metal_coop_task_t *tb = pm_metal_coop_create_task(&fb->coro);
    if (ta == NULL || tb == NULL) {
        return fail("task");
    }
...
static int32_t case_sleep_idle(void) {
    sleep_frame_t *f = (sleep_frame_t *)pm_metal_coop_coro_create(step_sleep, sizeof(*f));
    if (f == NULL) {
...
static int32_t case_nested_await(void) {
    nest_frame_t *f = (nest_frame_t *)pm_metal_coop_coro_create(step_nest, sizeof(*f));
    if (f == NULL) {
...
static int32_t case_deadlock_empty(void) {
    ...
    if (pm_metal_coop_run_until(&f->coro) == 0) {
...
static int32_t case_facade_yield(void) {
    (void)pm_metal_coop_yield();
    return 0;
}

int32_t pm_metal_coop_tests(void) {
    if (pm_metal_coop_n_runners() < 2u) {
        return fail("ncpu");
    }
...
PM_MOD_TEST_C(pymergetic.metal.coop, tests, pm_metal_coop_tests);
