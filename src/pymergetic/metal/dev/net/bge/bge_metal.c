...
/* efi: real EFI_PHYSICAL_ADDRESS/EFI_STATUS + gBS->AllocatePages;
 * bios: no boot-services page pool exists, always defers to the arena.
 * impl: src/{efi,bios}/pymergetic/metal/dev/net/bge/bge_metal_port.c */
...
#define DELAY(us) pm_metal_time_usleep((uint32_t)(us))
...
#endif
#if METAL_BGE_DEBUG
#define BGE_DBG(...) ((void)0)
static uint32_t s_bge_tx_ok;
static uint32_t s_bge_rx_ok;
static uint32_t s_bge_rx_err;
#else
#define BGE_DBG(...) \
  do {               \
  } while (0)
 ...
 */
static void bge_dma_sync(const void *addr, uintptr_t len)
{
  const uint8_t *p;
  uintptr_t      off;
...
static void *metal_bge_dma_alloc(uintptr_t bytes)
{
  void *pa;

  pa = pm_metal_bge_port_alloc_pages(bytes);
  if (pa != NULL) {
    memset(pa, 0, bytes);
    bge_dma_sync(pa, bytes);
    return pa;
  }
...
static uint64_t metal_bge_vtop(const void *p)
{
...
uint32_t bge_pci_read(metal_bge_softc_t *sc, int off)
{
  ...
  if (off == BGE_PCI_CMD) {
    return pm_metal_bus_pci_read16(sc->bus, sc->dev, sc->func, (uint8_t)off);
  }

  return pm_metal_bus_pci_read32(sc->bus, sc->dev, sc->func, (uint8_t)off);
}

void bge_pci_write(metal_bge_softc_t *sc, int off, uint32_t val)
{
  if (off == BGE_PCI_CMD) {
    pm_metal_bus_pci_write16(sc->bus, sc->dev, sc->func, (uint8_t)off, (uint16_t)val);
    return;
  }

  pm_metal_bus_pci_write32(sc->bus, sc->dev, sc->func, (uint8_t)off, val);
}

static uint32_t bge_readmem_ind(metal_bge_softc_t *sc, int off)
{
  bge_pci_write(sc, BGE_PCI_MEMWIN_BASEADDR, (uint32_t)off);
  return bge_pci_read(sc, BGE_PCI_MEMWIN_DATA);
}

static void bge_writemem_ind(metal_bge_softc_t *sc, int off, int val)
{
  bge_pci_write(sc, BGE_PCI_MEMWIN_BASEADDR, (uint32_t)off);
  bge_pci_write(sc, BGE_PCI_MEMWIN_DATA, (uint32_t)val);
}

void bge_writemem_direct(metal_bge_softc_t *sc, int off, int val)
{
  CSR_WRITE_4(sc, off, val);
}

static void bge_writereg_ind(metal_bge_softc_t *sc, int off, int val)
{
  bge_pci_write(sc, BGE_PCI_REG_BASEADDR, (uint32_t)off);
  bge_pci_write(sc, BGE_PCI_REG_DATA, (uint32_t)val);
}

static void bge_writembx(metal_bge_softc_t *sc, int off, int val)
{
  CSR_WRITE_4(sc, off, val);
...
static uint32_t bge_chipid_read(metal_bge_softc_t *sc, uint16_t did)
{
  uint32_t id;
...
static uint8_t bge_pcie_cap_off(metal_bge_softc_t *sc)
{
  uint8_t ptr;
  uint8_t id;
  int32_t guard;

  ptr = (uint8_t)(pm_metal_bus_pci_read8(sc->bus, sc->dev, sc->func, BGE_PCI_CAPPTR) & ~0x3u);
  for (guard = 0; ptr != 0 && guard < 48; guard++) {
    id = pm_metal_bus_pci_read8(sc->bus, sc->dev, sc->func, ptr);
    if (id == PCI_CAP_ID_PCIE) {
      return ptr;
    }

    ptr = (uint8_t)(pm_metal_bus_pci_read8(sc->bus, sc->dev, sc->func, ptr + 1u) & ~0x3u);
  }

  /* BCM575x often advertises PCIe at 0xD0. */
  if (pm_metal_bus_pci_read8(sc->bus, sc->dev, sc->func, BGE_PCIE_CAPID_REG) == BGE_PCIE_CAPID) {
    return (uint8_t)BGE_PCIE_CAPID_REG;
  }

  return 0;
}

static void bge_pcie_clear_nosnoop(metal_bge_softc_t *sc)
{
  uint8_t  cap;
  uint16_t devctl;
  ...
  devctl = pm_metal_bus_pci_read16(sc->bus, sc->dev, sc->func, (uint8_t)(cap + PCIER_DEVICE_CTL));
  devctl &= (uint16_t) ~(PCIEM_CTL_RELAXED_ORD_ENABLE | PCIEM_CTL_NOSNOOP_ENABLE);
  pm_metal_bus_pci_write16(sc->bus, sc->dev, sc->func, (uint8_t)(cap + PCIER_DEVICE_CTL), devctl);
}

static void bge_set_family_flags(metal_bge_softc_t *sc)
{
  switch (sc->asicrev) {
  ...
  case BGE_ASICREV_BCM57780:
    sc->flags |= BGE_FLAG_5755_PLUS | BGE_FLAG_575X_PLUS | BGE_FLAG_5705_PLUS;
    break;
  ...
  sc->bge_return_ring_cnt = BGE_IS_5705_PLUS(sc) ? BGE_RETURN_RING_CNT_5705 : BGE_RETURN_RING_CNT;
  sc->phy_addr            = 1;
  sc->mi_mode             = BGE_MIMODE_BASE;
}

static int bge_miibus_readreg(metal_bge_softc_t *sc, int reg)
{
  uint32_t val;
  int      i;

  CSR_WRITE_4(
    sc, BGE_MI_COMM, BGE_MICMD_READ | BGE_MICOMM_BUSY | BGE_MIPHY(sc->phy_addr) | BGE_MIREG(reg));
...
static void bge_miibus_writereg(metal_bge_softc_t *sc, int reg, int val)
{
  int i;

  CSR_WRITE_4(sc,
              BGE_MI_COMM,
              BGE_MICMD_WRITE | BGE_MICOMM_BUSY | BGE_MIPHY(sc->phy_addr) | BGE_MIREG(reg) |
                (uint32_t)val);
  for (i = 0; i < BGE_TIMEOUT; i++) {
    DELAY(10);
...
static void bge_mac_update_link(metal_bge_softc_t *sc)
{
  uint32_t mac_mode;
  int      aux;
  int      res;
   ...
   */
  mac_mode = CSR_READ_4(sc, BGE_MAC_MODE) & ~(BGE_MACMODE_PORTMODE | BGE_MACMODE_HALF_DUPLEX);
  gig      = 0;
  fdx      = 1;
  aux      = 0;
  ...
  DELAY(40);
  BGE_DBG(
    "metal-bge: macmode %s %s aux=0x%x", gig ? "gmii" : "mii", fdx ? "fdx" : "hdx", (uint32_t)aux);
}

static int bge_phy_init(metal_bge_softc_t *sc)
{
  int i;
  int bmcr;
  int bmsr;
  ...
  bge_miibus_writereg(sc, BRGPHY_MII_BMCR, BMCR_ANENABLE | BMCR_ANRESTART);
  DELAY(100000);
...
static int bge_mac_ok(const uint8_t *mac)
{
  uintptr_t i;
  int32_t   nonzero;
...
static void bge_mac_synthesize(metal_bge_softc_t *sc)
{
  sc->mac[0] = 0x02;
  sc->mac[1] = 0x00;
  sc->mac[2] = 0x57;
...
static int bge_eeprom_getbyte(metal_bge_softc_t *sc, int addr, uint8_t *dest)
{
  int      i;
  uint32_t byte;
  ...
  *dest = (uint8_t)((byte >> ((addr % 4) * 8)) & 0xffu);
  return 0;
}

static int bge_get_eaddr_mem(metal_bge_softc_t *sc, uint8_t mac[6])
{
  uint32_t hi;
  uint32_t lo;
...
static int bge_get_eaddr_eeprom(metal_bge_softc_t *sc, uint8_t mac[6])
{
  int i;
  int off;
...
static int bge_get_eaddr_regs(metal_bge_softc_t *sc, uint8_t mac[6])
{
  uint32_t lo;
  uint32_t hi;
  ...
  mac[0] = (uint8_t)((lo >> 8) & 0xffu);
  mac[1] = (uint8_t)(lo & 0xffu);
  mac[2] = (uint8_t)((hi >> 24) & 0xffu);
  mac[3] = (uint8_t)((hi >> 16) & 0xffu);
  mac[4] = (uint8_t)((hi >> 8) & 0xffu);
  mac[5] = (uint8_t)(hi & 0xffu);
  return bge_mac_ok(mac) ? 0 : -1;
}

static uint16_t bge_htons(uint16_t v)
{
  return (uint16_t)(((v & 0xffu) << 8) | ((v >> 8) & 0xffu));
}

static void bge_program_mac(metal_bge_softc_t *sc)
{
  uint16_t *m;

  m = (uint16_t *)sc->mac;
  CSR_WRITE_4(sc, BGE_MAC_ADDR1_LO, bge_htons(m[0]));
  CSR_WRITE_4(sc, BGE_MAC_ADDR1_HI, ((uint32_t)bge_htons(m[1]) << 16) | bge_htons(m[2]));
}

static void bge_read_mac(metal_bge_softc_t *sc)
{
  /*
...
static uint32_t bge_dma_swap_options(metal_bge_softc_t *sc)
{
   ...
   */
  dma_options =
    BGE_MODECTL_WORDSWAP_NONFRAME | BGE_MODECTL_BYTESWAP_DATA | BGE_MODECTL_WORDSWAP_DATA;
...
static int bge_reset(metal_bge_softc_t *sc)
{
  ...
  if (sc->flags & BGE_FLAG_PCIE) {
...
#if METAL_BGE_DEBUG
    {
      ...
      if (cap != 0) {
        devctl = pm_metal_bus_pci_read16(sc->bus, sc->dev, sc->func, (uint8_t)(cap + PCIER_DEVICE_CTL));
        BGE_DBG("metal-bge: pcie cap=0x%x devctl=0x%x", (uint32_t)cap, (uint32_t)devctl);
      } else {
...
static int bge_chipinit(metal_bge_softc_t *sc)
{
  uint32_t dma_rw_ctl;
  uint32_t mode_ctl;
  ...
  mode_ctl |= BGE_MODECTL_MAC_ATTN_INTR | BGE_MODECTL_HOST_SEND_BDS | BGE_MODECTL_TX_NO_PHDR_CSUM |
              BGE_MODECTL_STACKUP;
...
static int bge_dma_setup(metal_bge_softc_t *sc)
{
  sc->ldata.bge_rx_std_ring    = metal_bge_dma_alloc(BGE_STD_RX_RING_SZ);
  sc->ldata.bge_rx_return_ring = metal_bge_dma_alloc(BGE_RX_RTN_RING_SZ(sc));
  ...
  sc->ldata.bge_rx_std_ring_paddr    = metal_bge_vtop(sc->ldata.bge_rx_std_ring);
  sc->ldata.bge_rx_return_ring_paddr = metal_bge_vtop(sc->ldata.bge_rx_return_ring);
  sc->ldata.bge_tx_ring_paddr        = metal_bge_vtop(sc->ldata.bge_tx_ring);
  sc->ldata.bge_status_block_paddr   = metal_bge_vtop(sc->ldata.bge_status_block);
  sc->ldata.bge_stats_paddr          = metal_bge_vtop(sc->ldata.bge_stats);

  for (uintptr_t i = 0; i < BGE_STD_RX_RING_CNT; i++) {
...
static int bge_newbuf_std(metal_bge_softc_t *sc, int i)
{
  struct bge_rx_bd *r;
...
static int bge_init_rx_ring_std(metal_bge_softc_t *sc)
{
  memset(sc->ldata.bge_rx_std_ring, 0, BGE_STD_RX_RING_SZ);
  sc->std = 0;
...
static int bge_init_tx_ring(metal_bge_softc_t *sc)
{
  sc->txcnt            = 0;
  sc->tx_saved_considx = 0;
  sc->tx_prodidx       = 0;
  memset(sc->ldata.bge_tx_ring, 0, BGE_TX_RING_SZ);
  bge_writembx(sc, BGE_MBX_TX_HOST_PROD0_LO, 0);
  bge_writembx(sc, BGE_MBX_TX_NIC_PROD0_LO, 0);
  return 0;
}

static int bge_blockinit(metal_bge_softc_t *sc)
{
  struct bge_rcb *rcb;
  bge_hostaddr    taddr;
  bus_size_t      vrcb;
  ...
  rcb->bge_hostaddr.bge_addr_lo = (uint32_t)sc->ldata.bge_rx_std_ring_paddr;
  rcb->bge_hostaddr.bge_addr_hi = (uint32_t)(sc->ldata.bge_rx_std_ring_paddr >> 32);
  ...
  RCB_WRITE_4(sc, vrcb, bge_nicaddr, BGE_NIC_TXRING_ADDR(0, BGE_TX_RING_CNT));
  RCB_WRITE_4(sc, vrcb, bge_maxlen_flags, BGE_RCB_MAXLEN_FLAGS(BGE_TX_RING_CNT, 0));
  ...
  RCB_WRITE_4(sc, vrcb, bge_hostaddr.bge_addr_lo, taddr.bge_addr_lo);
  RCB_WRITE_4(sc, vrcb, bge_nicaddr, 0);
  RCB_WRITE_4(sc, vrcb, bge_maxlen_flags, BGE_RCB_MAXLEN_FLAGS(sc->bge_return_ring_cnt, 0));
  ...
  CSR_WRITE_4(sc, BGE_HCC_STATUSBLK_ADDR_HI, (uint32_t)(sc->ldata.bge_status_block_paddr >> 32));
  CSR_WRITE_4(sc, BGE_HCC_STATUSBLK_ADDR_LO, (uint32_t)sc->ldata.bge_status_block_paddr);
  memset(sc->ldata.bge_status_block, 0, 32);
  ...
  val = BGE_MACMODE_TXDMA_ENB | BGE_MACMODE_RXDMA_ENB | BGE_MACMODE_RX_STATS_CLEAR |
        BGE_MACMODE_TX_STATS_CLEAR | BGE_MACMODE_RX_STATS_ENB | BGE_MACMODE_TX_STATS_ENB |
  ...
  CSR_WRITE_4(sc,
              BGE_MAC_STS,
              BGE_MACSTAT_SYNC_CHANGED | BGE_MACSTAT_CFG_CHANGED | BGE_MACSTAT_MI_COMPLETE |
                BGE_MACSTAT_LINK_CHANGED);

  return 0;
}

int metal_bge_attach(metal_bge_softc_t *sc, uint8_t bus, uint8_t dev, uint8_t func)
{
  uint64_t bar;
  ...
  sc->regs    = (volatile uint8_t *)(uintptr_t)bar;
  sc->chipid  = bge_chipid_read(sc, pm_metal_bus_pci_read16(bus, dev, func, 0x02));
...
static void bge_rxreuse_std(metal_bge_softc_t *sc, int i)
{
  struct bge_rx_bd *r;
...
static void bge_rxeof(metal_bge_softc_t *sc, uint16_t rx_prod, metal_bge_rx_fn fn, void *ctx)
{
  ...
  while (rx_cons != rx_prod) {
    ...
    if (len > 14 && fn != NULL) {
      ...
      if (s_bge_rx_ok <= 4u || (s_bge_rx_ok % 32u) == 0u) {
        BGE_DBG(
          "metal-bge: rx=%u tx=%u err=%u len=%u", s_bge_rx_ok, s_bge_tx_ok, s_bge_rx_err, len);
      }
#endif
    }
  ...
  bge_writembx(sc, BGE_MBX_RX_STD_PROD_LO, (sc->std - 1) & (BGE_STD_RX_RING_CNT - 1));
}

static void bge_txeof(metal_bge_softc_t *sc, uint16_t tx_cons)
{
...
void metal_bge_poll(metal_bge_softc_t *sc, metal_bge_rx_fn fn, void *ctx)
{
  ...
  if (rx_prod != sc->rx_saved_considx) {
    bge_dma_sync(sc->ldata.bge_rx_return_ring,
                 (uintptr_t)sc->bge_return_ring_cnt * sizeof(struct bge_rx_bd));
  }
  ...
  if (s_bge_tx_ok != 0u && s_bge_rx_ok == 0u && (s_link_ticks % 256u) == 0u) {
    BGE_DBG("metal-bge: poll tx=%u rx=0 err=%u prod=%u txcons=%u link=%d"
...
int metal_bge_tx(metal_bge_softc_t *sc, const void *frame, uint32_t len)
{
  ...
  if (s_bge_tx_ok <= 8u) {
    ...
    BGE_DBG("metal-bge: tx=%u len=%u dst=%02x:%02x:%02x:%02x:%02x:%02x et=0x%x",
            s_bge_tx_ok,
            len,
            f[0],
            f[1],
