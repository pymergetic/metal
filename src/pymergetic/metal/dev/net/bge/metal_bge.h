...
#define CSR_READ_4(sc, off)       (*(volatile uint32_t *)((uint8_t *)(sc)->regs + (off)))
#define CSR_WRITE_4(sc, off, val) (CSR_READ_4((sc), (off)) = (uint32_t)(val))
...
#define RCB_WRITE_4(sc, rcb, field, val) \
  bge_writemem_direct((sc), (int)((rcb) + offsetof(struct bge_rcb, field)), (int)(val))

#define PCI_CLRBIT(sc, reg, x, sz) bge_pci_write((sc), (reg), bge_pci_read((sc), (reg)) & ~(x))
