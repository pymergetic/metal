...
{
  pm_metal_gfx_out16(VBE_DISPI_IOPORT_INDEX, index);
  pm_metal_gfx_out16(VBE_DISPI_IOPORT_DATA, value);
}

static uint16_t BochsVbeRead(uint16_t index)
{
  pm_metal_gfx_out16(VBE_DISPI_IOPORT_INDEX, index);
  return pm_metal_gfx_in16(VBE_DISPI_IOPORT_DATA);
}
...
static int32_t BochsProbe(const pm_metal_scanout_bind_t *b)
{
  ...
  if (pm_metal_bus_pci_find(0x1234, 0x1111, &bus, &dev, &func) != 0) {
    return -1;
  }
