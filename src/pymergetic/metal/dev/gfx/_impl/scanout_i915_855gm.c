...
static uint64_t I915BarSize(uint8_t bus, uint8_t dev, uint8_t func, uint8_t bar_index)
{
  ...
  save_lo = pm_metal_bus_pci_read32(bus, dev, func, off);
  if ((save_lo & 1u) != 0) {
    return 0;
  }
  ...
  save_hi = 0;
  if (is64) {
    save_hi = pm_metal_bus_pci_read32(bus, dev, func, (uint8_t)(off + 4));
  }

  pm_metal_bus_pci_write32(bus, dev, func, off, 0xFFFFFFFFu);
  if (is64) {
    pm_metal_bus_pci_write32(bus, dev, func, (uint8_t)(off + 4), 0xFFFFFFFFu);
  }

  lo = pm_metal_bus_pci_read32(bus, dev, func, off) & ~0xFu;
  hi = 0;
  if (is64) {
    hi = pm_metal_bus_pci_read32(bus, dev, func, (uint8_t)(off + 4));
  }

  pm_metal_bus_pci_write32(bus, dev, func, off, save_lo);
  if (is64) {
    pm_metal_bus_pci_write32(bus, dev, func, (uint8_t)(off + 4), save_hi);
  }
...
static int32_t I915EmitBlit(int32_t x, int32_t y, int32_t w, int32_t h)
{
  ...
  b = pm_metal_dev_gfx_scanout_bind_info();
...
static int32_t I915Probe(const pm_metal_scanout_bind_t *b)
{
  ...
  if (pm_metal_bus_pci_find(I915_855GM_VENDOR, I915_855GM_DEVICE, &bus, &dev, &func) != 0) {
    return -1;
  }

  pm_metal_bus_pci_enable_mem_bm(bus, dev, func);

  mmio_bar = pm_metal_bus_pci_bar_mmio(bus, dev, func, 0, &cons);
  if (mmio_bar == 0) {
    return -1;
  }

  /* GMADR is the large prefetchable aperture (usually BAR2 on 855GM). */
  apert_bar = pm_metal_bus_pci_bar_mmio(bus, dev, func, 2, &cons);
  apert_sz  = I915BarSize(bus, dev, func, 2);
  if (apert_bar == 0 || apert_sz < 8u * 1024u * 1024u) {
    apert_bar = pm_metal_bus_pci_bar_mmio(bus, dev, func, 1, &cons);
