/* pymergetic.metal.jit.rs — host prove.
 *
...
static int32_t test_compile_alloc(void) {
    ...
    if (coro->step != pm_metal_jit_rs_compile_step) {
        pm_util_mem_arena_destroy(arena);
        free(backing);
        return 4;
    }
...
static int32_t test_compile_error(void) {
    ...
    pm_metal_jit_rs_result_free(NULL, NULL);
    pm_util_mem_arena_destroy(arena);
    free(backing);
    return 0;
}
...
static int32_t pm_metal_jit_rs_tests(void) {
...
PM_MOD_TEST_C(pymergetic.metal.jit.rs, tests, pm_metal_jit_rs_tests);