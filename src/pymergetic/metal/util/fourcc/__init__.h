/*
 * FourCC tags — semantic BE u32 + LE wire bytes.
 * Static tags: PM_METAL_UTIL_FOURCC('A','B','C','D')
 */
...
/* Big-endian FourCC (semantic tag value); matches from_string(). */
#define PM_METAL_UTIL_FOURCC(a, b, c, d)                             \
  (((uint32_t)(uint8_t)(a) << 24) | ((uint32_t)(uint8_t)(b) << 16) | \
   ((uint32_t)(uint8_t)(c) << 8) | (uint32_t)(uint8_t)(d))

/* LE wire value — byte0..3 spell the tag in on-wire blobs. */
...
int pm_metal_util_fourcc_to_string(const pm_metal_util_fourcc_t *tag,
