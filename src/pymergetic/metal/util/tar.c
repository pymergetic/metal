...
static pm_metal_util_tar_state_t *pm_metal_util_tar_writer_state(
  const pm_metal_util_tar_writer_t *w)
{
  pm_metal_util_tar_slot_t *s;
...
int pm_metal_util_tar_iter_name(const pm_metal_util_tar_iter_t *it, char *out, size_t cap)
{
  ...
  for (n = 0; n < sizeof(st->hdr.name) && st->hdr.name[n] != '\0'; n++) {
  }
  if (n + 1 > cap) {
    return -1;
  }
...
static void pm_metal_util_tar_iter_init_native(wasm_exec_env_t           exec_env,
...
static int32_t pm_metal_util_tar_iter_next_native(wasm_exec_env_t           exec_env,
                                                  pm_metal_util_tar_iter_t *it)
{
  (void)exec_env;
...
static int32_t pm_metal_util_tar_iter_read_native(wasm_exec_env_t           exec_env,
                                                  pm_metal_util_tar_iter_t *it,
                                                  void                     *dst,
                                                  uint32_t                  dst_cap)
{
...
static void pm_metal_util_tar_iter_close_native(wasm_exec_env_t           exec_env,
                                                pm_metal_util_tar_iter_t *it)
{
  (void)exec_env;
...
static NativeSymbol g_pm_metal_util_tar_native_symbols[] = {
  ...
  { "pm_metal_util_tar_writer_put_header",
    (void *)pm_metal_util_tar_writer_put_header_native,
    "(*$Ii)i",
    NULL },
  { "pm_metal_util_tar_writer_put_data",
    (void *)pm_metal_util_tar_writer_put_data_native,
    "(**~)i",
    NULL },
  { "pm_metal_util_tar_writer_put", (void *)pm_metal_util_tar_writer_put_native, "(*$i*~)i", NULL },
  { "pm_metal_util_tar_writer_finish",
    (void *)pm_metal_util_tar_writer_finish_native,
    "(*)I",
    NULL },
};
...
{
  if (!wasm_runtime_register_natives(PM_METAL_UTIL_TAR_WASI_MODULE,
                                     g_pm_metal_util_tar_native_symbols,
