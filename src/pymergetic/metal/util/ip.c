  ...
  for (;;) {
    ...
    *n = v;
    ...
    if (*p == '\0') {
      ...
      *n = v;
      if (out_host != NULL) {
        *out_host = ((uint32_t)a << 24) | ((uint32_t)b << 16) | ((uint32_t)c << 8) | (uint32_t)d;
      }
...
static int Ip4PutOctet(char *out, size_t cap, size_t *off, unsigned v)
{
  char     tmp[3];
  unsigned n;
  unsigned i;
...
static int32_t pm_metal_util_ip4_parse_native(wasm_exec_env_t exec_env, char *s, uint32_t *out_host)
{
  (void)exec_env;
  return (int32_t)pm_metal_util_ip4_parse(s, out_host);
}

static int32_t pm_metal_util_ip4_format_native(wasm_exec_env_t exec_env,
                                               uint32_t        host,
                                               char           *out,
                                               uint32_t        cap)
{
...
static NativeSymbol g_pm_metal_util_ip_native_symbols[] = {
  { "pm_metal_util_ip4_parse", (void *)pm_metal_util_ip4_parse_native, "($*)i", NULL },
  { "pm_metal_util_ip4_format", (void *)pm_metal_util_ip4_format_native, "(i*~)i", NULL },
};

int pm_metal_util_ip_native_register(void)
{
  if (!wasm_runtime_register_natives(PM_METAL_UTIL_IP_WASI_MODULE,
                                     g_pm_metal_util_ip_native_symbols,
