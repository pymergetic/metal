...
__attribute__((weak)) pm_metal_ui_handle_t pm_metal_wasm_stdout_tab(void)
{
...
static void emit_line(const char *line, int also_serial)
{
  ...
  if (also_serial) {
    /*
         ...
         */
    if (pm_metal_port_owned() || pm_metal_input_focus() == PM_METAL_INPUT_FOCUS_GUEST)
...
static bool handle_is_stdio(os_file_handle handle)
{
  return handle == STDIN_FILENO || handle == STDOUT_FILENO || handle == STDERR_FILENO;
}

static efi_wamr_file_t *file_from_fd(os_file_handle handle)
{
  int idx;
...
__wasi_errno_t os_fstatat(os_file_handle            handle,
                          const char               *path,
...
__wasi_errno_t os_openat(os_file_handle             handle,
                         const char                *path,
                         ...
                         os_file_handle            *out)
{
  ...
  if (handle != EFI_WAMR_FD_PREOPEN)
    return __WASI_EBADF;
  if ((oflags & __WASI_O_CREAT) || (oflags & __WASI_O_TRUNC) || (oflags & __WASI_O_DIRECTORY))
    return __WASI_ENOTCAPABLE;
  if (!pm_metal_esp_ready())
    return __WASI_ENOENT;
...
__wasi_errno_t os_file_get_access_mode(os_file_handle              handle,
                                       wasi_libc_file_access_mode *access_mode)
{
  if (access_mode == NULL)
    return __WASI_EFAULT;
...
__wasi_errno_t os_close(os_file_handle handle, bool is_stdio_fd)
{
  efi_wamr_file_t *f;
...
__wasi_errno_t os_preadv(os_file_handle               handle,
                         const struct __wasi_iovec_t *iov,
                         int                          iovcnt,
...
__wasi_errno_t os_readv(os_file_handle               handle,
                        const struct __wasi_iovec_t *iov,
                        int                          iovcnt,
                        size_t                      *nread)
{
  ...
  for (i = 0; i < iovcnt; i++) {
    ...
    if (want > avail)
      want = avail;
    if (want > 0) {
      memcpy(iov[i].buf, f->data + f->pos, want);
      f->pos += (uint32_t)want;
      total += want;
    }
  }
                         ...
                         size_t                       *nwritten)
{
  ...
  for (i = 0; i < iovcnt; i++) {
    if (iov[i].buf == NULL && iov[i].buf_len > 0)
      return __WASI_EFAULT;
    efi_wamr_feed_stdout((const char *)iov[i].buf, (size_t)iov[i].buf_len, 1);
...
__wasi_errno_t os_fallocate(os_file_handle    handle,
                            __wasi_filesize_t offset,
...
__wasi_errno_t os_utimensat(os_file_handle       handle,
                            const char          *path,
                            __wasi_timestamp_t   access_time,
                            __wasi_timestamp_t   modification_time,
                            __wasi_fstflags_t    fstflags,
...
__wasi_errno_t os_readlinkat(
  os_file_handle handle, const char *path, char *buf, size_t bufsize, size_t *nread)
{
  (void)handle;
  (void)path;
  (void)buf;
...
__wasi_errno_t os_linkat(os_file_handle       from_handle,
                         const char          *from_path,
                         os_file_handle       to_handle,
                         const char          *to_path,
                         __wasi_lookupflags_t lookup_flags)
{
  (void)from_handle;
...
__wasi_errno_t os_renameat(os_file_handle old_handle,
                           const char    *old_path,
                           os_file_handle new_handle,
                           const char    *new_path)
{
  (void)old_handle;
  (void)old_path;
...
__wasi_errno_t os_lseek(os_file_handle     handle,
...
__wasi_errno_t os_fadvise(os_file_handle    handle,
                          __wasi_filesize_t offset,
                          __wasi_filesize_t length,
...
os_dir_stream os_get_invalid_dir_stream(void)
{
  return NULL;
}
