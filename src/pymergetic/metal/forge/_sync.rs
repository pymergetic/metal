...
/// The `include/pymergetic/metal/...` root every generated face lands under.
fn include_root(metal_root: &str) -> String {
    join_path(metal_root, "include")
}

/// Mirrored `include/` directory for a module's `src/` directory: strips
/// the `src` root, then reparents under `include/`. This is the *only*
/// place any generated face for this module is ever written -- a
/// module's own `src/` directory holds human input (`_impl/`, `.pm/`)
/// only.
fn out_dir_for(mod_dir: &str, roots: &[String], metal_root: &str) -> String {
    join_path(&include_root(metal_root), &mod_dir_rel(mod_dir, roots))
}
...
/// Banner-owned faces directly under `dir` (a module's `include/` mirror).
/// Everything under there is generated by construction, but this still
/// checks the banner defensively before treating anything as prunable.
fn list_generated_at<S: ForgeStore>(store: &mut S, dir: &str) -> Vec<String> {
...
fn impl_sources<S: ForgeStore>(store: &mut S, mod_dir: &str, impl_lang: &str) -> Vec<String> {
    ...
    // `_impl/` holds human source when present; every generated face for
    // this module lands under the separate `include/pymergetic/metal/<mod>/`
    // tree regardless (see out_dir_for), never back into `mod_dir`. Falls
    // back to `mod_dir` directly for modules not yet migrated to `_impl/`.
...
fn remove_marked<S: ForgeStore>(store: &mut S, dir: &str, rel: &str) -> bool {
...
fn sync_stem<S: ForgeStore>(
    ...
    for slot in &emit_now {
        match convert_stem_slot(store, mod_dir, out_dir, meta, human_path, slot, force) {
            Ok(a) => {
    ...
    for slot in POOL {
        ...
        if remove_marked(store, out_dir, &rel) {
            pruned.push(rel);
        }
    }
...
pub fn cmd_sync<S: ForgeStore, Sess: ForgeSession>(
    ...
    for mod_dir in &mods {
        ...
        for human in &sources {
            match sync_stem(store, mod_dir, &out_dir, &meta, human, &slots, &roots, force) {
        ...
        if ok {
            // prune orphan faces (human stem deleted since last sync)
            let keep: Vec<String> = sources
                ...
                .collect();
            for rel in list_generated_at(store, &out_dir) {
                let stem = rel.rsplit_once('.').map(|(s, _)| s).unwrap_or(&rel);
                if !keep.iter().any(|k| k == stem) {
                    let _ = remove_marked(store, &out_dir, &rel);
                }
            }
        }
        if ok {
    ...
    for mod_dir in &mods {
        match load_meta(store, mod_dir) {
            Ok(meta) => {
                ...
                for rel in &sources {
                    face_errs +=
                        check_face_symmetry(store, sess, &out_dir, &rel_mod, rel, &meta);
                }
