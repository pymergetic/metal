...
#define PCI_CLASSCODE_OFFSET       0x09u
...
{
  pm_metal_io_out32(0xCF8, Addr(bus, dev, func, offset));
  return pm_metal_io_in32(0xCFC);
}

uint16_t pm_bios_pci_read16(uint8_t bus, uint8_t dev, uint8_t func, uint8_t offset)
{
  uint32_t v = pm_bios_pci_read32(bus, dev, func, offset);
...
{
  uint16_t cmd = pm_bios_pci_read16(bus, dev, func, PCI_COMMAND_OFFSET);
  ...
  uint8_t bus, uint8_t dev, uint8_t func, uint8_t bar_index, uint8_t *bars_consumed)
{
  ...
  uint32_t lo  = pm_bios_pci_read32(bus, dev, func, off);
  uint64_t base;
  ...
  if (((lo >> 1) & 3u) == 2u) {
    uint32_t hi = pm_bios_pci_read32(bus, dev, func, (uint8_t)(off + 4));
    base |= (uint64_t)hi << 32;
    if (bars_consumed != NULL)
      *bars_consumed = 2;
  }
...
{
  uint8_t  off = (uint8_t)(PCI_BAR0 + bar_index * 4);
  uint32_t bar = pm_bios_pci_read32(bus, dev, func, off);
  ...
  uint16_t vendor, uint16_t device, uint8_t *bus_out, uint8_t *dev_out, uint8_t *func_out)
{
  ...
  for (bus = 0; bus < 256; bus++) {
    for (dev = 0; dev < 32; dev++) {
      ...
      ven = pm_bios_pci_read16((uint8_t)bus, dev, 0, PCI_VENDOR_ID_OFFSET);
      if (ven == 0xffff) {
        continue;
      }

      hdr  = pm_bios_pci_read8((uint8_t)bus, dev, 0, PCI_HEADER_TYPE_OFFSET);
      fmax = (hdr & HEADER_TYPE_MULTI_FUNCTION) ? 8 : 1;
      for (func = 0; func < fmax; func++) {
        ven = pm_bios_pci_read16((uint8_t)bus, dev, func, PCI_VENDOR_ID_OFFSET);
        if (ven != vendor) {
          continue;
        }

        if (pm_bios_pci_read16((uint8_t)bus, dev, func, PCI_DEVICE_ID_OFFSET) == device) {
          if (bus_out != NULL) {
            *bus_out = (uint8_t)bus;
          }
  ...
  for (bus = 0; bus < 256; bus++) {
    for (dev = 0; dev < 32; dev++) {
      ...
      for (func = 0; func < fmax; func++) {
        ...
        id  = pm_bios_pci_read32((uint8_t)bus, dev, func, PCI_REVISION_ID_OFFSET);
