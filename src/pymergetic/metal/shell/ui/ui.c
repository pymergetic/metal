/** @file
  UI — IRIX chrome, multi-tab, console + input. Handle ABI. (impl: efi|bios)
**/
  ...
  )
{
  ...
  CopyMem (
    &prompt[plen],
    mSysConsole->u.console.input,
    mSysConsole->u.console.input_len
    );
  plen += mSysConsole->u.console.input_len;
  if (mSysConsole->u.console.cursor_on) {
    prompt[plen++] = 0xDB;
  }
  ...
  )
{
  ...
  /* Chrome always paints in screen space (DEFAULT surface). Caller presents. */
  pm_metal_gfx_set_surface (PM_METAL_GFX_SURFACE_DEFAULT);
  MetalUiLayout ();
  MetalUiPaint ();
  return 0;
...
{
  if (mShellRoot == NULL || mSysConsole == NULL) {
    return -1;
  }
