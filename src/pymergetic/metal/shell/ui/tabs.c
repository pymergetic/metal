...
pm_metal_ui_handle_t pm_metal_ui_tab_open(const char *title, int activate)
{
  ...
  if (activate) {
    gMetalUiTabs->u.tabs.active = gMetalUiTabs->u.tabs.n;
    pm_metal_lifecycle_set(tab->surface != 0 ? tab->surface : PM_METAL_GFX_SURFACE_DEFAULT,
  ...
  for (i = 0; i < gMetalUiTabs->u.tabs.n; i++) {
    ...
    if (gMetalUiTabs->u.tabs.n == 0) {
      ...
      gMetalUiTabs->u.tabs.active = 0;
      {
        ...
        if (next != NULL) {
          pm_metal_lifecycle_set(next->surface != 0 ? next->surface : PM_METAL_GFX_SURFACE_DEFAULT,
                                 PM_METAL_LIFE_FOCUSED | PM_METAL_LIFE_VISIBLE);
        }
      }
    } else if (was > i) {
    ...
    if (gMetalUiTabs->u.tabs.hover == (int32_t)i) {
      gMetalUiTabs->u.tabs.hover = -1;
    } else if (gMetalUiTabs->u.tabs.hover > (int32_t)i) {
...
void pm_metal_ui_set_status(const char *text)
{
  ...
  snprintf(gMetalUiStatus->u.status.text, sizeof(gMetalUiStatus->u.status.text), "%s", text);
}
...
int pm_metal_ui_tab_cycle(int delta)
{
  ...
  next = (int32_t)a + delta;
  while (next < 0) {
    next += (int32_t)n;
  }

  next %= (int32_t)n;
  return pm_metal_ui_tab_activate_index((uint32_t)next);
}
...
int pm_metal_ui_tab_content_rect(
  pm_metal_ui_handle_t tab_h, int32_t *ox, int32_t *oy, int32_t *ow, int32_t *oh)
{
  metal_ui_widget_t *tab;
