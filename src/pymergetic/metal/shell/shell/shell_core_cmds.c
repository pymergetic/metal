...
PM_METAL_SHELL_CMDS(g_pm_metal_shell_cmds_core) = {
  ...
  { "unload", "unload <mod>      drop idle mod (refused if busy)", CoreUnloadCmd },
  { "mods", "mods              list loaded mod registry slots", CoreModsCmd },
  ...
  { "exit", "exit|quit [-r] [-f]  power off (-r reboot, -f skip sleeps)", CoreExitCmd },
  { "quit", "exit|quit [-r] [-f]  power off (-r reboot, -f skip sleeps)", CoreExitCmd },
  { "shutdown", "exit|quit [-r] [-f]  power off (-r reboot, -f skip sleeps)", CoreExitCmd },
};
