...
void pm_metal_util_fourcc_from_wire_bytes(const uint8_t bytes[PM_METAL_UTIL_FOURCC_LEN],
...
int pm_metal_util_fourcc_to_string(const pm_metal_util_fourcc_t *tag, char out[PM_METAL_UTIL_FOURCC_LEN + 1])
{
	...
	pm_metal_util_fourcc_to_wire_bytes(tag, (uint8_t *)out);
...
int pm_metal_util_fourcc_from_string(const char *s, pm_metal_util_fourcc_t *out)
{
	...
	pm_metal_util_fourcc_from_wire_bytes((const uint8_t *)s, out);
	return 0;
}
