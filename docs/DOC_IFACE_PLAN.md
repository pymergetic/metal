...
| 2026-07-28 00:05 | cursor-agent (composer) | DONE III+smoke | Live smoke green. Fixes along the way: (1) `pm_metal_py_bind_t` pad to 64 so aligned(16) section stride matches pointer math — DOC fields had made sizeof 56 and most PY_BINDs never installed (`ImportError: pymergetic.metal.fs`); (2) curated ESP `mods/py` stage (no microdot_src) so ESP cache does not drop then-current stdlib.zip (later replaced by loose `/mods/py/stdlib` + `py@metal.stdlib`); (3) then `utemplate.zip`/`templates.zip` + sys.path (ESP import_stat has no DIR; product trees later went fully loose); (4) list `?limit=` because sync `asgi conn_send` aborts large bodies. Smoke: `scripts/verify.d/port/efi/doc-iface-smoke.sh`. |
