...
`build_exported()` walks `vars` and mallocs a plain `"NAME=VALUE"` array containing only the entries whose `exported` is nonzero — passed straight into `spawn()`'s own `envc`/`envp`, no conversion needed. Proven end-to-end (a genuinely spawned child, not just the array shape) by `tests/process_test.c` spawning `mods/t2_env.wasm` twice with the same built env — once asking for a local-only var (must print `(unset)`) and once for the exported one (must print its real value) — see `scripts/verify-linux-process.sh`.
...
- [x] WASI preview1's own socket extension (wasi1, not preview2), address-pool-gated, proven with loopback TCP/UDP/IPv6 + localhost resolve across spawned processes — see "Sockets" above, `mods/t10`–`t11` / `t24`–`t28`, `scripts/verify-linux-process.sh` and Zephyr verify
