...
| `util/doc` | `include/…/doc.h` | `src/common/…/util/doc.c` — `impl: common` + its own wasi-import bridge; read-only adapter over the shell live table, `PY_BIND`/`PY_BIND_DOC` linker section, and mod `register_func_doc` records — never a fourth doc store (see docs/DOC_IFACE_PLAN.md Part I) |
| `util/iface` | `include/…/iface.h` | `src/common/…/util/iface.c` (+ `iface_embed_install.c`, `iface_esp_install.c`) — `impl: common` + its own wasi-import bridge; header-pack registry (lz4(ustar) via `util/lz4` + `util/tar`), ESP `mods/apps/*/iface.list` sidecars, and the `NativeSymbol` sym table generated by `scripts/gen_iface_syms.py` (see docs/DOC_IFACE_PLAN.md Part II, docs/IFACE.md) |
| `runtime/mem/limit` | `include/…/runtime/mem/limit.h` | `src/…/runtime/mem/limit.c` — compile-time mem/buffer budget catalog (`PM_METAL_MEM_LIMIT` / `.pm_metal_mem_limits.*`); see docs/IO.md |
...
`util/doc` and `util/iface` are both read-only **catalogs over existing
authority**, not new stores: `doc.*` walks the shell live table / `PY_BIND`
linker section / mod func-doc records that each already own their own
text, and `iface.*` walks the `NativeSymbol[]` arrays that each wasi-import
bridge (`util/size.c`, `util/ip.c`, `fs.c`, …) already declares for its own
guest ABI — see docs/DOC_IFACE_PLAN.md for the full design and
docs/IFACE.md for the header-pack + sym-table shape.
...
Compile-time budgets, iface embed toggles, and default
`scripts/build` target/post-action come from Kconfig
([`config/`](../config/), menus mirror `pymergetic.metal` modules) —
see [`KCONFIG.md`](KCONFIG.md). `scripts/confgen` emits
`build/autoconf.h` + `build/config.sh`; argv/env override for one call.
