...
shared APIs). No `runtime/` nesting — modules sit flat under
...
```text
src/pymergetic/metal/boot/          # orchestration + harvest
...
src/pymergetic/metal/console/       # CONCEPT — ring + viewports (not hardware)
src/pymergetic/metal/dev/serial/    # HARDWARE — unified serial over platform uart
src/pymergetic/metal/dt/
src/pymergetic/metal/mem/
```
...
```text
src/pymergetic/metal/dt/     # .module — sealed add/lookup/walk
```
...
target entry (bios|efi)
  ...
  -> boot tree print
  # proofs + ping/http/ntp: ./forge-cli stress only
```
