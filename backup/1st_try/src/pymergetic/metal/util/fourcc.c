#include "pymergetic/kernel/util/fourcc.h"
...
void pm_metal_util_fourcc_to_bytes(const pm_metal_util_fourcc_Tag *tag, uint8_t out[PM_METAL_UTIL_FOURCC_LEN])
{
...
int pm_metal_util_fourcc_to_string(const pm_metal_util_fourcc_Tag *tag, char out[PM_METAL_UTIL_FOURCC_LEN + 1])
{
    if (tag == NULL || out == NULL) {
        return -1;
    }
    pm_metal_util_fourcc_to_bytes(tag, (uint8_t *)out);
...
int pm_metal_util_fourcc_from_string(const char *s, pm_metal_util_fourcc_Tag *out)
{
    ...
    pm_metal_util_fourcc_from_u32(out, pack_be((uint8_t)s[0], (uint8_t)s[1], (uint8_t)s[2], (uint8_t)s[3]));
    return 0;
}
