...
#include "zephyr_wasi_platform.h"

/* WAMR platform_internal.h stubs int/void* handle types; keep ours above. */
...
resolve_open_path(os_file_handle handle, const char *path, char *abs_path, size_t abs_path_len)
{
	...
	if (path[0] == '/') {
		...
		if (len2 + 1 > abs_path_len) {
			abs_path[0] = '\0';
			return false;
		}

		memcpy(abs_path, path, len2 + 1);
		return true;
	}

	if (handle != NULL && !os_is_virtual_fd(handle->fd)) {
	...
	if (len1 > 0 && base[len1 - 1] == '/') {
		snprintf(abs_path, abs_path_len, "%s%s", base, path);
	} else {
		snprintf(abs_path, abs_path_len, "%s/%s", base, path);
	}
...
static struct zephyr_handle g_stdin_handle = { .fd = STDIN_FILENO, .is_sock = false };
static struct zephyr_handle g_stdout_handle = { .fd = STDOUT_FILENO, .is_sock = false };
static struct zephyr_handle g_stderr_handle = { .fd = STDERR_FILENO, .is_sock = false };

os_file_handle
os_convert_stdin_handle(os_raw_file_handle raw_stdin)
{
	(void)raw_stdin;
	return &g_stdin_handle;
}
