...
int pm_metal_orchestrator_boot(const pm_metal_sys_bootstrap_t *blob)
{
	if (blob == NULL) {
		return 1;
	}

	if (pm_metal_sys_bootstrap_bind(blob) != 0) {
		fprintf(stderr, "orchestrator: pm_metal_sys_bootstrap_bind failed\n");
		return 1;
	}

	printf("pymergetic orchestrator\n");
	printf("  built = %llu\n", (unsigned long long)pm_metal_sys_built());
