/* Metal L2 NIC face — register virtio/bge ops for µPy / bring-up. */

#ifndef PM_METAL_NET_NIC_H_
#define PM_METAL_NET_NIC_H_
...
typedef struct pm_metal_net_nic_l2_ops {
    int (*open)(uint8_t mac_out[6]);
    const uint8_t *(*mac)(void);
    int (*tx)(const void *frame, uint32_t len);
    void (*poll)(void);
} pm_metal_net_nic_l2_ops_t;

/* Record L2 ops. Returns 0 on success, <0 on error. */
