 ...
 *
 * impl: common — src/pymergetic/metal/bus/virtio/virtio_pci.c
 */
#ifndef PYMERGETIC_METAL_BUS_VIRTIO_VIRTIO_H_
#define PYMERGETIC_METAL_BUS_VIRTIO_VIRTIO_H_
...
/** Page-aligned DMA memory that survives ExitBootServices (EfiLoaderData on EFI). */
void *pm_metal_virtio_pages_alloc(unsigned pages);
void  pm_metal_virtio_pages_free(void *buf, unsigned pages);

/** Bytes -> 4 KiB page count, rounded up (arg for pm_metal_virtio_pages_alloc). */
#define PM_METAL_VIRTIO_SIZE_TO_PAGES(sz) (((sz) + 4095u) / 4096u)

/** Re-publish queue rings after ExitBootServices (resets avail/used indices). */
int pm_metal_virtio_queue_reset(pm_metal_virtio_dev_t *dev, uint16_t qidx);

/** Remap MMIO + renegotiate features + reset queue(s) after ExitBootServices. */
...
/** PCI presence scan only (no device init). */
...
#endif /* PYMERGETIC_METAL_BUS_VIRTIO_VIRTIO_H_ */
