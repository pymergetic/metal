 ...
 *
 * impl: common — src/pymergetic/metal/runtime/async/async.c
 */
...
/** Host-only: park a host coro on another async handle (DNS, connect, …). */
int32_t pm_metal_async_await_coro(pm_metal_coro_t *self,
				  pm_metal_async_handle_t aw_h);

/** Host-only: resolve handle → underlying coro (NULL if guest task handle). */
