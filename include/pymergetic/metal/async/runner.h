...
/*
 * N CPUs = N equal runners. Product metal refuses n<2 (ACPI + SMP).
 * After pm_metal_smp_start(), each CPU polls only its runner
 * (see async/smp.h). Before SMP online, run_poll drains all (BSP).
 */
...
/* Advance ready work; after SMP, current CPU only. */
