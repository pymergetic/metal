 ...
 *
 * impl: src/pymergetic/metal/boot/externals.c
 */
...
/* Host registry (firmware + Emscripten). Guest WASI imports: PM_METAL_WASM_GUEST. */
...
#define PM_METAL_EXTERNAL(var, id, version, url, note)                         \
    static const pm_metal_external_t var##_row = {                             \
        #id, (version), (url), (note)                                          \
    };                                                                         \
...
/** Section bounds — walked by externals.c (PROVIDE_HIDDEN in link.ld). */
...
/** Seed dyn table when linker section is empty (UEFI PE / no INSERT script). */
void pm_metal_externals_init(void);
/** Implemented in externals_rows.c — same ids as PM_METAL_EXTERNAL rows. */
