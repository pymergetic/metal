...
typedef struct pm_metal_port_ops {
  ...
  void (*set_framebuffer)(void *fb, unsigned width, unsigned height, unsigned ppsl);
  int (*get_framebuffer)(void **fb, unsigned *width, unsigned *height, unsigned *ppsl);
} pm_metal_port_ops_t;

/** Single linked port provides this table. */
extern const pm_metal_port_ops_t pm_metal_port_ops;

static inline int pm_metal_port_owned(void)
{
  return pm_metal_port_ops.owned();
}

static inline int pm_metal_port_takeover_and_run(void *image_handle, unsigned n_cpus)
{
  return pm_metal_port_ops.takeover_and_run(image_handle, n_cpus);
}

static inline void pm_metal_port_reset(int reboot)
{
  pm_metal_port_ops.reset(reboot);
}

static inline void pm_metal_port_set_framebuffer(void    *fb,
                                                 unsigned width,
                                                 unsigned height,
                                                 unsigned ppsl)
{
...
static inline int pm_metal_port_get_framebuffer(void    **fb,
                                                unsigned *width,
                                                unsigned *height,
                                                unsigned *ppsl)
{
