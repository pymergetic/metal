...
#define PM_METAL_FS_IMPORT(name) PM_METAL_WASI_IMPORT(PM_METAL_FS_WASI_MODULE, name)

extern pm_metal_async_handle_t pm_metal_fs_open_async(const char *path, uint32_t flags)
...
extern pm_metal_async_handle_t pm_metal_fs_fread_async(pm_metal_fs_h h, uint32_t dest, uint32_t len)
  PM_METAL_FS_IMPORT(pm_metal_fs_fread_async);
extern pm_metal_async_handle_t pm_metal_fs_fwrite_async(pm_metal_fs_h h, uint32_t src, uint32_t len)
...
extern pm_metal_async_handle_t pm_metal_fs_stat_async(const char *path, uint32_t dest)
  PM_METAL_FS_IMPORT(pm_metal_fs_stat_async);
extern pm_metal_async_handle_t pm_metal_fs_fstat_async(pm_metal_fs_h h, uint32_t dest)
...
extern pm_metal_async_handle_t pm_metal_fs_rename_async(const char *old_path, const char *new_path)
...
/** Awaitable read into a host mem cookie (survives coro unpin). */
extern pm_metal_async_handle_t pm_metal_fs_read_mem_async(const char    *path,
                                                          pm_metal_ptr_t dest,
                                                          uint32_t       dest_len)
  PM_METAL_FS_IMPORT(pm_metal_fs_read_mem_async);
extern pm_metal_async_handle_t pm_metal_fs_write_mem_async(const char    *path,
                                                           pm_metal_ptr_t src,
                                                           uint32_t       src_len)
  PM_METAL_FS_IMPORT(pm_metal_fs_write_mem_async);
extern uint32_t pm_metal_fs_result(pm_metal_async_handle_t self_h)
  PM_METAL_FS_IMPORT(pm_metal_fs_result);

extern uint32_t pm_metal_fs_size(const char *path) PM_METAL_FS_IMPORT(pm_metal_fs_size);
extern uint32_t pm_metal_fs_read(const char *path, uint32_t dest, uint32_t dest_len)
  PM_METAL_FS_IMPORT(pm_metal_fs_read);
extern uint32_t pm_metal_fs_write(const char *path, uint32_t src, uint32_t src_len)
...
pm_metal_async_handle_t pm_metal_fs_write_async(const char *path,
                                                const void *src,
                                                uint32_t    src_len);
/** Awaitable read/write into a guest mem cookie (host TLSF). */
