...
static uint32_t pm_metal_ui_console_handle_native(wasm_exec_env_t exec_env)
{
  (void)exec_env;
  return (uint32_t)pm_metal_ui_console_handle();
}

static uint32_t pm_metal_ui_tab_open_native(wasm_exec_env_t exec_env, char *title, int32_t activate)
{
  (void)exec_env;
  return (uint32_t)pm_metal_ui_tab_open(title, (int)activate);
}

static int32_t pm_metal_ui_tab_close_native(wasm_exec_env_t exec_env, uint32_t h)
{
  (void)exec_env;
  return (int32_t)pm_metal_ui_tab_close((pm_metal_ui_handle_t)h);
}

static int32_t pm_metal_ui_tab_activate_native(wasm_exec_env_t exec_env, uint32_t h)
{
  (void)exec_env;
  return (int32_t)pm_metal_ui_tab_activate((pm_metal_ui_handle_t)h);
}

static uint32_t pm_metal_ui_tab_count_native(wasm_exec_env_t exec_env)
{
  (void)exec_env;
  return (uint32_t)pm_metal_ui_tab_count();
}

static uint32_t pm_metal_ui_tab_active_native(wasm_exec_env_t exec_env)
{
  (void)exec_env;
  return (uint32_t)pm_metal_ui_tab_active();
}

static void pm_metal_ui_tab_puts_native(wasm_exec_env_t exec_env, uint32_t h, char *line)
{
  (void)exec_env;
  pm_metal_ui_tab_puts((pm_metal_ui_handle_t)h, line);
}

static void pm_metal_ui_console_puts_native(wasm_exec_env_t exec_env, char *line)
{
  (void)exec_env;
  pm_metal_ui_console_puts(line);
}

static void pm_metal_ui_active_puts_native(wasm_exec_env_t exec_env, char *line)
{
  (void)exec_env;
  pm_metal_ui_active_puts(line);
}

static void pm_metal_ui_set_status_native(wasm_exec_env_t exec_env, char *text)
{
  (void)exec_env;
  pm_metal_ui_set_status(text);
}

static void pm_metal_ui_input_clear_native(wasm_exec_env_t exec_env)
{
  (void)exec_env;
  pm_metal_ui_input_clear();
}

static int32_t pm_metal_ui_input_append_native(wasm_exec_env_t exec_env, int32_t ch)
{
  (void)exec_env;
  return (int32_t)pm_metal_ui_input_append((char)ch);
}

static int32_t pm_metal_ui_input_backspace_native(wasm_exec_env_t exec_env)
{
  (void)exec_env;
...
static int32_t pm_metal_ui_input_set_native(wasm_exec_env_t exec_env, char *text)
{
  (void)exec_env;
  return (int32_t)pm_metal_ui_input_set(text);
}

static uint32_t pm_metal_ui_tab_surface_native(wasm_exec_env_t exec_env, uint32_t h)
{
  (void)exec_env;
...
{
  if (!wasm_runtime_register_natives(PM_METAL_UI_WASI_MODULE,
