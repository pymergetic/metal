...
int os_socket_accept(bh_socket_t server_sock, bh_socket_t *sock, void *addr, unsigned int *addrlen)
{
  (void)server_sock;
  (void)sock;
  (void)addr;
...
int os_socket_recv_from(
  bh_socket_t socket, void *buf, unsigned int len, int flags, bh_sockaddr_t *src_addr)
{
  (void)socket;
  (void)buf;
  (void)len;
...
int os_socket_send_to(
  bh_socket_t socket, const void *buf, unsigned int len, int flags, const bh_sockaddr_t *dest_addr)
{
  (void)socket;
  (void)buf;
  (void)len;
...
int os_socket_addr_resolve(const char     *host,
                           const char     *service,
                           uint8_t        *hint_is_tcp,
                           uint8_t        *hint_is_ipv4,
                           bh_addr_info_t *addr_info,
                           size_t          addr_info_size,
