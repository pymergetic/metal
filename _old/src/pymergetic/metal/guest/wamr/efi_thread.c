...
int os_thread_create(korp_tid              *p_tid,
...
int os_thread_create_with_prio(
  korp_tid *p_tid, thread_start_routine_t start, void *arg, unsigned int stack_size, int prio)
{
  (void)p_tid;
  (void)start;
  (void)arg;
...
{
  (void)usec;
  /* Blocking sleep omitted on Metal — use pm_metal_async_sleep_us + await. */
  return BHT_ERROR;
}
