...
uint32_t bge_pci_read(metal_bge_softc_t *sc, int off)
{
  ...
  if (off == BGE_PCI_CMD) {
    return pm_bios_pci_read16(sc->bus, sc->dev, sc->func, (uint8_t)off);
  }

  return pm_bios_pci_read32(sc->bus, sc->dev, sc->func, (uint8_t)off);
}

void bge_pci_write(metal_bge_softc_t *sc, int off, uint32_t val)
{
  if (off == BGE_PCI_CMD) {
    pm_bios_pci_write16(sc->bus, sc->dev, sc->func, (uint8_t)off, (uint16_t)val);
    return;
  }

  pm_bios_pci_write32(sc->bus, sc->dev, sc->func, (uint8_t)off, val);
}
...
static uint8_t bge_pcie_cap_off(metal_bge_softc_t *sc)
{
  ...
  ptr = (uint8_t)(pm_bios_pci_read8(sc->bus, sc->dev, sc->func, BGE_PCI_CAPPTR) & ~0x3u);
  for (guard = 0; ptr != 0 && guard < 48; guard++) {
    ...
    ptr = (uint8_t)(pm_bios_pci_read8(sc->bus, sc->dev, sc->func, ptr + 1u) & ~0x3u);
  }

  /* BCM575x often advertises PCIe at 0xD0. */
  if (pm_bios_pci_read8(sc->bus, sc->dev, sc->func, BGE_PCIE_CAPID_REG) == BGE_PCIE_CAPID) {
...
static void bge_pcie_clear_nosnoop(metal_bge_softc_t *sc)
{
  ...
  devctl = pm_bios_pci_read16(sc->bus, sc->dev, sc->func, (uint8_t)(cap + PCIER_DEVICE_CTL));
  devctl &= (uint16_t) ~(PCIEM_CTL_RELAXED_ORD_ENABLE | PCIEM_CTL_NOSNOOP_ENABLE);
  pm_bios_pci_write16(sc->bus, sc->dev, sc->func, (uint8_t)(cap + PCIER_DEVICE_CTL), devctl);
}
...
int metal_bge_attach(metal_bge_softc_t *sc, uint8_t bus, uint8_t dev, uint8_t func)
{
  ...
  sc->chipid  = bge_chipid_read(sc, pm_bios_pci_read16(bus, dev, func, 0x02));
