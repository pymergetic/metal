...
void *pm_metal_util_arena_alloc(pm_metal_util_arena_t *arena, size_t size)
{
  ...
  if (b->size >= need + block_hdr + PM_METAL_UTIL_ARENA_ALIGN) {
    ...
    {
      pm_metal_util_arena_block_t *rest = (pm_metal_util_arena_block_t *)(arena->base + rest_off);
      ...
      if (rest->next_off != PM_METAL_UTIL_ARENA_OFF_NULL) {
        pm_metal_util_arena_block_t *n = pm_metal_util_arena_block_at(arena, rest->next_off);
...
void pm_metal_util_arena_free(pm_metal_util_arena_t *arena, void *ptr)
{
  ...
  if (b->next_off != PM_METAL_UTIL_ARENA_OFF_NULL) {
    ...
    if (n && !n->used && n->magic == PM_METAL_UTIL_ARENA_MAGIC_FREE) {
      ...
      if (b->next_off != PM_METAL_UTIL_ARENA_OFF_NULL) {
        pm_metal_util_arena_block_t *nn = pm_metal_util_arena_block_at(arena, b->next_off);
  ...
  if (b->prev_off != PM_METAL_UTIL_ARENA_OFF_NULL) {
    ...
    if (p && !p->used && p->magic == PM_METAL_UTIL_ARENA_MAGIC_FREE) {
      ...
      if (p->next_off != PM_METAL_UTIL_ARENA_OFF_NULL) {
        pm_metal_util_arena_block_t *nn = pm_metal_util_arena_block_at(arena, p->next_off);
...
static int32_t pm_metal_util_arena_init_native(wasm_exec_env_t exec_env,
                                               void           *buf,
                                               uint32_t        buf_len)
{
  pm_metal_util_arena_t *arena;
...
static int32_t pm_metal_util_arena_alloc_native(wasm_exec_env_t exec_env,
                                                uint32_t        handle,
                                                uint32_t        size)
{
...
{
  if (!wasm_runtime_register_natives(PM_METAL_UTIL_ARENA_WASI_MODULE,
                                     g_pm_metal_util_arena_native_symbols,
