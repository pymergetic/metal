...
void *pm_metal_virtio_pages_alloc(unsigned pages)
{
  ...
  return pm_metal_arena_map((size_t)pages * PM_METAL_MEM_PAGE_SIZE);
}
...
void pm_metal_virtio_pages_free(void *buf, unsigned pages)
{
  ...
  (void)pm_metal_arena_unmap(buf, (size_t)pages * PM_METAL_MEM_PAGE_SIZE);
}
  ...
  while (Ptr >= 0x40 && Ptr != 0xff) {
    ...
    if (Id == VIRTIO_PCI_CAP_ID_VENDOR) {
      ...
      for (i = 0; i < sizeof(Buf); i++) {
        Buf[i] = pm_bios_pci_read8(Bus, Dev, Func, (uint8_t)(Ptr + i));
      }
    ...
    Ptr = pm_bios_pci_read8(Bus, Dev, Func, (uint8_t)(Ptr + 1));
  }
  ...
  uint8_t Bus, uint8_t Dev, uint8_t Func, uint16_t WantId, pm_metal_virtio_dev_t *Out)
{
  ...
  p = (metal_vdev_priv_t *)pm_metal_mem_alloc(sizeof(*p), PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
  if (p == NULL) {
    return PM_ST_UNSUPPORTED;
  }
...
int pm_metal_virtio_setup_queue(pm_metal_virtio_dev_t *dev, uint16_t qidx, uint16_t want_size)
{
  ...
  if (p->Vqs == NULL) {
    p->Vqs = (pm_metal_virtq_t *)pm_metal_mem_alloc(
      sizeof(pm_metal_virtq_t) * 8u, PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
    if (p->Vqs == NULL) {
      return -1;
    }
  ...
  vq->last_used  = 0;
  vq->next =
    (uint16_t *)pm_metal_mem_alloc(sizeof(uint16_t) * Qsz, PM_METAL_MEM_HEAP, PM_METAL_MEM_ID_NONE);
  if (vq->next == NULL) {
