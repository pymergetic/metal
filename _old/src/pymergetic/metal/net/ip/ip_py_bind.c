/** @file
  ...
  domain/type are net.h's raw PM_METAL_NET_IP_AF_* / PM_METAL_NET_IP_SOCK_* ints
...
/*
 * pm_metal_net_ip_recv(h, ptr, len) writes into `ptr` for the *entire* async
...
static net_py_recv_slot_t *NetPyRecvSlotFor(pm_metal_net_ip_sock_h h)
{
  uint32_t i;
  int32_t  free_idx = -1;
  ...
  for (i = 0; i < NET_PY_RECV_SLOTS; i++) {
    ...
    if (mRecvSlots[i].sock == PM_METAL_NET_IP_SOCK_INVALID && free_idx < 0) {
      free_idx = (int32_t)i;
    }
  }
...
static void NetPyRecvSlotFree(pm_metal_net_ip_sock_h h)
{
  uint32_t i;

  for (i = 0; i < NET_PY_RECV_SLOTS; i++) {
    if (mRecvSlots[i].sock == h) {
      mRecvSlots[i].sock = PM_METAL_NET_IP_SOCK_INVALID;
      return;
    }
  }
}
...
PM_METAL_PY_BIND(
  g_py_bind_net_socket, "pymergetic.metal.net.ip", "socket", py_net_socket_obj, PM_METAL_PY_SYNC);
...
PM_METAL_PY_BIND(
  g_py_bind_net_send, "pymergetic.metal.net.ip", "send", py_net_send_obj, PM_METAL_PY_SYNC);
...
PM_METAL_PY_BIND(
  g_py_bind_net_close, "pymergetic.metal.net.ip", "close", py_net_close_obj, PM_METAL_PY_SYNC);
...
PM_METAL_PY_BIND(
  g_py_bind_net_listen, "pymergetic.metal.net.ip", "listen", py_net_listen_obj, PM_METAL_PY_SYNC);
...
PM_METAL_PY_BIND(
  g_py_bind_net_accept, "pymergetic.metal.net.ip", "accept", py_net_accept_obj, PM_METAL_PY_SYNC);
...
static mp_obj_t py_net_recv(mp_obj_t h_obj, mp_obj_t n_obj)
{
  ...
  ah = pm_metal_net_ip_recv(h, slot->buf, want);
  if (ah == PM_METAL_ASYNC_HANDLE_INVALID) {
...
PM_METAL_PY_BIND(
  g_py_bind_net_recv, "pymergetic.metal.net.ip", "recv", py_net_recv_obj, PM_METAL_PY_SYNC);
...
{
  pm_metal_async_handle_t ah = pm_metal_net_ip_dns(mp_obj_str_get_str(host_obj));
...
PM_METAL_PY_BIND(
  g_py_bind_net_dns, "pymergetic.metal.net.ip", "dns", py_net_dns_obj, PM_METAL_PY_SYNC);
...
static mp_obj_t py_net_dns_last_ntoa(void)
{
  ...
  if (pm_metal_net_ip_dns_last_ntoa(addr, sizeof(addr)) != 0) {
    return pm_metal_py_obj_none();
  }
