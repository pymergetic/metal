...
static void metal_dhcp6_stateful_set(metal_dhcp6_stateful_t *st, u8_t state)
{
  if (st == NULL) {
    return;
  }
...
static err_t metal_dhcp6_sf_pcb_get(void)
{
...
static void metal_dhcp6_sf_pcb_put(void)
{
  if (g_dhcp6_sf_pcb_refs == 0) {
    return;
  }
...
static u16_t metal_dhcp6_put_opt(
  u8_t *out, u16_t off, u16_t max, u16_t type, const u8_t *data, u16_t len)
{
...
static u16_t metal_dhcp6_append_duid_ll(u8_t *out, u16_t off, u16_t max, struct netif *netif)
{
  u8_t duid[10];
  ...
  duid[2] = 0;
  duid[3] = 1;
  memcpy(&duid[4], netif->hwaddr, 6);
  return metal_dhcp6_put_opt(out, off, max, DHCP6_OPTION_CLIENTID, duid, sizeof(duid));
}

static u16_t metal_dhcp6_append_ia_na(u8_t *out, u16_t off, u16_t max, u32_t iaid)
{
  u8_t ia[12];
...
static err_t metal_dhcp6_send(struct netif           *netif,
                              metal_dhcp6_stateful_t *st,
                              u8_t                    msgtype,
                              u8_t                    with_server_id)
{
  ...
  if (with_server_id && st->server_id_len > 0) {
    olen = metal_dhcp6_put_opt(
      opts, olen, sizeof(opts), DHCP6_OPTION_SERVERID, st->server_id, st->server_id_len);
  }

  olen = metal_dhcp6_append_ia_na(opts, olen, sizeof(opts), st->iaid);
  memcpy((u8_t *)(msg + 1), opts, olen);
...
static s8_t metal_dhcp6_addr_slot(struct netif *netif)
{
  s8_t i;
...
static void metal_dhcp6_apply_addr(struct netif           *netif,
                                   metal_dhcp6_stateful_t *st,
...
static void metal_dhcp6_parse_ia_addr(struct netif           *netif,
                                      metal_dhcp6_stateful_t *st,
                                      const u8_t             *data,
                                      u16_t                   len)
{
  ...
  memcpy(&a6, (void *)data, sizeof(struct ip6_addr_packed));
  ip6_addr_assign_zone(&a6, IP6_UNICAST, netif);
  metal_dhcp6_apply_addr(netif, st, &a6);
}

static void metal_dhcp6_parse_options(
  struct netif *netif, metal_dhcp6_stateful_t *st, const u8_t *opts, u16_t opts_len, u8_t msgtype)
{
  u16_t off;
  ...
  while (off + 4u <= opts_len) {
    ...
    if (opt == DHCP6_OPTION_SERVERID && st->server_id_len == 0 && olen <= sizeof(st->server_id)) {
      memcpy(st->server_id, (void *)(opts + off), olen);
...
static void metal_dhcp6_sf_recv(
  void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
  ...
  msg = (struct dhcp6_msg *)p->payload;
  xid = ((u32_t)msg->transaction_id[0] << 16) | ((u32_t)msg->transaction_id[1] << 8) |
  ...
  metal_dhcp6_parse_options(
    ctx->netif, ctx->st, (const u8_t *)msg + opts_off, opts_len, msg->msgtype);
...
static metal_dhcp6_rx_ctx_t *metal_dhcp6_rx_ctx_for(struct netif *netif)
{
  uint32_t i;
