...
static void TlsReadBytesFn(void *ctx, const uint8_t **out_ptr, size_t *out_len)
{
  ...
  *out_ptr = pm_metal_net_tls_conn_read_buf(c->ch);
...
{
  return pm_metal_py_int_new((int64_t)pm_metal_net_tls_conn_open());
}
static MP_DEFINE_CONST_FUN_OBJ_0(py_tls_open_obj, py_tls_open);
PM_METAL_PY_BIND(
  g_py_bind_tls_open, "pymergetic.metal.net.tls", "open", py_tls_open_obj, PM_METAL_PY_SYNC);
...
static mp_obj_t py_tls_connect(size_t n_args, const mp_obj_t *args)
{
  ...
  ah = pm_metal_net_tls_conn_connect(ch, host, port, use_tls);
...
static mp_obj_t py_tls_write(mp_obj_t h_obj, mp_obj_t data_obj)
{
  ...
  ah = pm_metal_net_tls_conn_write(ch, data, (uint32_t)len);
  if (ah == PM_METAL_ASYNC_HANDLE_INVALID) {
...
PM_METAL_PY_BIND(
  g_py_bind_tls_write, "pymergetic.metal.net.tls", "write", py_tls_write_obj, PM_METAL_PY_SYNC);
...
static mp_obj_t py_tls_read(mp_obj_t h_obj, mp_obj_t n_obj)
{
  ...
  ah = pm_metal_net_tls_conn_read(ch, want);
  if (ah == PM_METAL_ASYNC_HANDLE_INVALID) {
...
PM_METAL_PY_BIND(
  g_py_bind_tls_read, "pymergetic.metal.net.tls", "read", py_tls_read_obj, PM_METAL_PY_SYNC);

static mp_obj_t py_tls_close(mp_obj_t h_obj)
{
  pm_metal_net_tls_conn_h ch = (pm_metal_net_tls_conn_h)pm_metal_py_int_get(h_obj);

  pm_metal_net_tls_conn_close(ch);
  return pm_metal_py_obj_none();
}
static MP_DEFINE_CONST_FUN_OBJ_1(py_tls_close_obj, py_tls_close);
PM_METAL_PY_BIND(
  g_py_bind_tls_close, "pymergetic.metal.net.tls", "close", py_tls_close_obj, PM_METAL_PY_SYNC);
