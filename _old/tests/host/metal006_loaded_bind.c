...
static int verify_loaded(const char *loaded, const char *ok_path, int ok_sig_valid)
{
...
int main(void)
{
  ...
  if (verify_loaded("EFI/BOOT/BOOTX64.EFI", "EFI/BOOT/BOOTX64.EFI", 1) != 0) {
    fprintf(stderr, "metal006: rejected loaded candidate\n");
    return 1;
  }
  /* Alter only the executed candidate — fail. */
  if (verify_loaded("EFI/BOOT/BOOTX64.EFI", "EFI/BOOT/BOOTX64.EFI", 0) != -1) {
