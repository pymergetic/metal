...
int main(void)
{
  ...
  pm_metal_util_log_write(
    PM_METAL_LOG_STREAM_STDOUT, PM_METAL_LOG_ERROR, "t3_util_native: at/above floor (expected)");
  ...
  int archive_packed_len = pm_metal_util_lz4_compress(
    archive, (size_t)archive_len, archive_packed, sizeof(archive_packed));
  if (archive_packed_len < 0) {
    printf("t3_util_native: tar archive compress failed\n");
    return 1;
  }
  printf(
    "t3_util_native: tar archive lz4 %" PRId64 " -> %d bytes\n", archive_len, archive_packed_len);
  ...
  while ((rc = pm_metal_util_tar_iter_next(&it)) == 1) {
    ...
    if (!is_dir) {
      ...
      while (got < entry_size) {
        int n = pm_metal_util_tar_iter_read(&it, entry_data + got, sizeof(entry_data) - got);
        if (n <= 0) {
