...
// In-process embedding (the metal jit.rs card) sets this to true so that a
// user's compile error throws a catchable exception instead of abort()ing the
// whole host process. The standalone CLI leaves it false and keeps the
// abort()-on-error behaviour unchanged.
bool g_span_throw_on_error = false;
...
void Span::bug(::std::function<void(::std::ostream&)> msg) const
{
    ...
    if (g_span_throw_on_error) {
        throw ::std::runtime_error("mrustc internal error (BUG)");
    }
#ifndef _WIN32
    abort();
#else
...
void Span::error(ErrorType tag, ::std::function<void(::std::ostream&)> msg) const {
    ...
    if (g_span_throw_on_error) {
        throw ::std::runtime_error("mrustc compile error");
    }
#ifndef _WIN32
    abort();
#else
