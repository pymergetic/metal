...
// Set true by in-process embedders (the metal jit.rs card) so Span::error() and
// Span::bug() throw instead of abort()ing the host. The CLI leaves it false.
extern bool g_span_throw_on_error;
